...
FB_MACHINEHOMINGCYCLE::FB_MACHINEHOMINGCYCLE()
    : MACHINEHOMINGSTEP(E_MACHINEHOMINGSTEP::IDLE), MACHINEHOMINGSTEPATERROR(E_MACHINEHOMINGSTEP::IDLE), CAUSE_HOMING_LOST_IN_MOTION(0), CAUSE_TX_ABORT_TOP_LOST(1), CAUSE_TX_ABORT_MOTION(2), CAUSE_HOMING_ERROR_M1(3), CAUSE_HOMING_ERROR_M2(4), CAUSE_DOUBLE_CONFIRM(5), CAUSE_CONFIRM_WHILE_HOMING(6)
{
    // Initialize variables
}

void FB_MACHINEHOMINGCYCLE::operator()() {
    if (__mocked_) { __mock_state_.call_count++; return; }
    M1DEMAND.HOMEREQ = false;
    M2DEMAND.HOMEREQ = false;
    BUCKETCOMMIT.COMMITOPEN = false;
    BUCKETCOMMIT.COMMITCLOSE = false;
    M2DEMAND.USEDYNAMICTARGET = ((PENDINGOPEN) | (PENDINGCLOSE)) | (HOMINGSTARTED);
    if (PENDINGCLOSE) {
    ...
    DOUBLECONFIRMREQUESTED = (CONFIRMOPENEDGE.Q) & (CONFIRMCLOSEEDGE.Q);
    SINGLECONFIRM = (CONFIRMOPENEDGE.Q) ^ (CONFIRMCLOSEEDGE.Q);
    CONFIRMWHILEHOMING = (SINGLECONFIRM) & (((M1STATUS.HOMINGBUSY) | (M2STATUS.HOMINGBUSY)));
    WINDOWSAFE = (((MODE == E_MODE::MAINT_N2)) & (TOPPOSITIONACTIVE)) & (WINCHESMECHANICALLYSTOPPED);
    AXISHOMED = (M1STATUS.HOMEDANDRELIABLE) & (M2STATUS.HOMEDANDRELIABLE);
    COMMITOROFFSETVALID = (COMMITPUBLISHED) | (BUCKETOFFSETVALID);
    MACHINEHOMEDRAW = (((AXISHOMED) & (COMMITOROFFSETVALID)) & (!MACHINEHOMINGFAILED)) & (!REHOMINGACKREQUIRED);
    if (MACHINEHOMEDRAW) {
        MACHINEWASHOMED = true;
    }
    if (((MACHINEWASHOMED) & (!MACHINEHOMEDRAW)) & (!WINCHESMECHANICALLYSTOPPED)) {
        HOMINGLOSSLATCHED = true;
    }
    if ((!MACHINEHOMEDRAW) & (WINCHESMECHANICALLYSTOPPED)) {
        MACHINEWASHOMED = false;
    }
    ...
    INSTCAUSES.at(CAUSE_HOMING_LOST_IN_MOTION).ACTIVE = (HOMINGLOSSLATCHED) | (REHOMINGACKREQUIRED);
    INSTCAUSES.at(CAUSE_HOMING_LOST_IN_MOTION).LATCHING = true;
    INSTCAUSES.at(CAUSE_HOMING_LOST_IN_MOTION).TEXTE = "Perte de datum machine en mouvement : re-referencer";
    INSTCAUSES.at(CAUSE_TX_ABORT_TOP_LOST).ACTIVE = (HOMINGSTARTED) & (!TOPPOSITIONACTIVE);
    INSTCAUSES.at(CAUSE_TX_ABORT_TOP_LOST).LATCHING = true;
    INSTCAUSES.at(CAUSE_TX_ABORT_TOP_LOST).TEXTE = "Capteur haut perdu pendant le referencement conjoint";
    INSTCAUSES.at(CAUSE_TX_ABORT_MOTION).ACTIVE = (HOMINGSTARTED) & (!WINCHESMECHANICALLYSTOPPED);
    INSTCAUSES.at(CAUSE_TX_ABORT_MOTION).LATCHING = true;
    INSTCAUSES.at(CAUSE_TX_ABORT_MOTION).TEXTE = "Mouvement detecte pendant le referencement conjoint";
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M1).ACTIVE = (HOMINGSTARTED) & (M1STATUS.HOMINGERROR);
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M1).LATCHING = true;
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M1).TEXTE = "Erreur de homing codeur M1 pendant le referencement conjoint";
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M2).ACTIVE = (HOMINGSTARTED) & (M2STATUS.HOMINGERROR);
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M2).LATCHING = true;
    INSTCAUSES.at(CAUSE_HOMING_ERROR_M2).TEXTE = "Erreur de homing codeur M2 pendant le referencement conjoint";
    INSTCAUSES.at(CAUSE_DOUBLE_CONFIRM).ACTIVE = DOUBLECONFIRMREQUESTED;
    INSTCAUSES.at(CAUSE_DOUBLE_CONFIRM).LATCHING = true;
    INSTCAUSES.at(CAUSE_DOUBLE_CONFIRM).TEXTE = "Confirmation benne Ouverte et Fermee simultanee : refusee";
    INSTCAUSES.at(CAUSE_CONFIRM_WHILE_HOMING).ACTIVE = CONFIRMWHILEHOMING;
    INSTCAUSES.at(CAUSE_CONFIRM_WHILE_HOMING).LATCHING = true;
    INSTCAUSES.at(CAUSE_CONFIRM_WHILE_HOMING).TEXTE = "Confirmation benne pendant un homing codeur en cours : ignoree";
    ...
    if (((((((SINGLECONFIRM) & (!HOMINGSTARTED)) & (!DOUBLECONFIRMREQUESTED)) & (!CONFIRMWHILEHOMING)) & (WINDOWSAFE)) & (!M1STATUS.HOMINGBUSY)) & (!M2STATUS.HOMINGBUSY)) {
    ...
    if (HOMINGSTARTED) {
        ...
        TRANSACTIONABORT = ((((!TOPPOSITIONACTIVE)) | ((!WINCHESMECHANICALLYSTOPPED))) | (M1STATUS.HOMINGERROR)) | (M2STATUS.HOMINGERROR);
        if (TRANSACTIONABORT) {
            MACHINEHOMINGFAILED = true;
            HOMINGSTARTED = false;
            PENDINGOPEN = false;
            PENDINGCLOSE = false;
        } else if ((M1STATUS.HOMINGBUSY) | (M2STATUS.HOMINGBUSY)) {
            HOMINGBUSYOBSERVED = true;
        } else if (((((HOMINGBUSYOBSERVED) & (M1STATUS.HOMEDANDRELIABLE)) & (M2STATUS.HOMEDANDRELIABLE)) & (M1STATUS.HOMINGDONE)) & (M2STATUS.HOMINGDONE)) {
    ...
    COMMITOROFFSETVALID = (COMMITPUBLISHED) | (BUCKETOFFSETVALID);
    MACHINEHOMED = ((((AXISHOMED) & (COMMITOROFFSETVALID)) & (!MACHINEHOMINGFAILED)) & (!REHOMINGACKREQUIRED)) & (!FAULT.LATCHED);
    if (LATCHEDEDGE.Q) {
        MACHINEHOMINGSTEPATERROR = MACHINEHOMINGSTEP;
    }
    if (MACHINEHOMINGFAILED) {
    ...
    } else if (((HOMINGSTARTED) | (M1STATUS.HOMINGBUSY)) | (M2STATUS.HOMINGBUSY)) {
        ...
        } else if (!WINCHESMECHANICALLYSTOPPED) {
