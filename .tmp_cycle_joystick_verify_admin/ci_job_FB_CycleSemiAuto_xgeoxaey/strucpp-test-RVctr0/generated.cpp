    ...
    if (!WAITINGRESUME) {
        switch (STATE) {
            ...
            case E_AUTOCYCLESTEP::AX3_OPEN_BUCKET:
                ...
                if ((((BENNE_ISOPEN) | (BENNE_DONE))) & (!BENNE_BUSY)) {
                    BUCKETCMD.REQOPEN = false;
                }
                BUCKETOPENSTABTIMER.IN = (((BENNE_ISOPEN) | (BENNE_DONE))) & (!BENNE_BUSY);
                BUCKETOPENSTABTIMER.PT = CST_BUCKETOPENSTABTIME;
                BUCKETOPENSTABTIMER();
                BUCKETOPENSTABTIMER.ENO = true;
                if (BUCKETOPENSTABTIMER.Q) {
                    WAITINGFOROPERATOR = !JOYSTICKDEFLECTED;
                    WAITINGFORPROCESS = false;
                    OPERATORACTIONID = 0x0403;
                    OPERATORACTION = "AX3 - Relacher puis repousser le joystick pour lancer la plongee.";
                    if (JOYDEFLECTEDEDGE.Q) {
                        STATE = E_AUTOCYCLESTEP::AX4_DESCEND_DIVING;
                    }
                }
                break;
            case E_AUTOCYCLESTEP::AX4_DESCEND_DIVING:
                ...
                BUCKETCMD.REQKOBOLDMEASUREENABLE = false;
                if ((JOYSTICKDEFLECTED) & ((M1_CABLEPOSM <= DIVESTARTMINEFF))) {
                    STATE = E_AUTOCYCLESTEP::AX5_KOBOLD_INIT;
                }
                break;
