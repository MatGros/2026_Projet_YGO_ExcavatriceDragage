...
FB_CYCLESEMIAUTO::FB_CYCLESEMIAUTO()
    : SAMPLECOUNTDONE(false), STATE(E_AUTOCYCLESTEP::AX0_REPOS), SAVEDSTATE(E_AUTOCYCLESTEP::AX0_REPOS), PAUSEDSTATE(E_AUTOCYCLESTEP::AX0_REPOS), WAITINGRESUME(false), CTRLASCENTDISTM(2.0), CTRLASCENTTOLERANCEM(0.25), CTRLASCENTTIMEOUT(30000000000LL), DRAININGTIME(5000000000LL), STEPMAXTIMEOUT(60000000000LL), CST_KOBOLDSTABTIME(500000000LL), CST_BOTTOMTOUCHSTABTIME(2000000000LL), CST_STEPSLOW(1), CST_STEPDIVE(4), CST_STEPLOADED(4), CST_KOBOLDINITTIMEOUT(3000000000LL), CST_DIVESTARTSTOPCONFIRMTIME(300000000LL), CST_DIVESTARTSTOPTIMEOUT(5000000000LL), CST_DIVESTARTSPEEDTHRESHOLDMPS(0.02), CST_RAISEOFFBOTTOMDEFAULTM(0.5), CST_COUPLEDPOSBACKLASHM(1.5)
{
    // Initialize variables
}

void FB_CYCLESEMIAUTO::operator()() {
    if (__mocked_) { __mock_state_.call_count++; return; }
    RESETEDGE.CLK = RESET;
    RESETEDGE();
    RESETEDGE.ENO = true;
    ...
    JOYDEFLECTEDEDGE();
    JOYDEFLECTEDEDGE.ENO = true;
    DIVESTARTSTOPPED = (((((((M1_SPEEDVALID) & (M2_SPEEDVALID)) & (M1_CONTACTORSRELEASED)) & (M2_CONTACTORSRELEASED)) & (M1_BRAKEAPPLIED)) & (M2_BRAKEAPPLIED)) & ((ABS(M1_MEASUREDSPEEDMPS) < CST_DIVESTARTSPEEDTHRESHOLDMPS))) & ((ABS(M2_MEASUREDSPEEDMPS) < CST_DIVESTARTSPEEDTHRESHOLDMPS));
    DIVESTARTSTOPTIMER.IN = ((STATE == E_AUTOCYCLESTEP::AX3_WAIT_DIVE_START)) & (DIVESTARTSTOPPED);
    DIVESTARTSTOPTIMER.PT = CST_DIVESTARTSTOPCONFIRMTIME;
    DIVESTARTSTOPTIMER();
    DIVESTARTSTOPTIMER.ENO = true;
    DIVESTARTTIMEOUTTIMER.IN = (STATE == E_AUTOCYCLESTEP::AX3_WAIT_DIVE_START);
    DIVESTARTTIMEOUTTIMER.PT = CST_DIVESTARTSTOPTIMEOUT;
    DIVESTARTTIMEOUTTIMER();
    DIVESTARTTIMEOUTTIMER.ENO = true;
    if (((DIVESTARTMINM >= 0.5)) & ((DIVESTARTMINM <= 8.0))) {
        DIVESTARTMINEFF = DIVESTARTMINM;
    } else {
        DIVESTARTMINEFF = 4.0;
    }
    RAISEOFFBOTTOMEFF = SEL(CFGRAISEOFFBOTTOMM > 0.0, CST_RAISEOFFBOTTOMDEFAULTM, CFGRAISEOFFBOTTOMM);
    CTRLASCENTMAXSTEPEFF = LIMIT(static_cast<IEC_INT>(1), CFGCTRLASCENTMAXSTEP, static_cast<IEC_INT>(2));
    CYCLERUNTIMER.IN = LIFECYCLE.BUSY;
    CYCLERUNTIMER.PT = 43200000000000LL;
    CYCLERUNTIMER();
    CYCLERUNTIMER.ENO = true;
    CURRENTCYCLEELAPSED = CYCLERUNTIMER.ET;
    DRAINTIMEEFF = SEL(CFGDRAINTIME <= 0LL, CFGDRAINTIME, DRAININGTIME);
    STABILIZATIONTIMER.IN = false;
    STABILIZATIONTIMER.PT = CTRLASCENTTIMEOUT;
    STABILIZATIONTIMER();
    STABILIZATIONTIMER.ENO = true;
    KOBOLDINITTIMER.IN = false;
    KOBOLDINITTIMER.PT = CST_KOBOLDINITTIMEOUT;
    KOBOLDINITTIMER();
    KOBOLDINITTIMER.ENO = true;
    SPEEDMISMATCHMPS = ABS(M1_MEASUREDSPEEDMPS - M2_MEASUREDSPEEDMPS);
    SPEEDMISMATCHACTIVE = (((((STATE == E_AUTOCYCLESTEP::AX11_CTRL_ASCENT)) & (JOYSTICKDEFLECTED)) & ((SPEEDMISMATCHTHRESHOLDMPS > 0.0))) & ((SPEEDMISMATCHTIMEOUT > 0LL))) & ((SPEEDMISMATCHMPS > SPEEDMISMATCHTHRESHOLDMPS));
    ...
    STEPMAXTIMER.IN = false;
    STEPMAXTIMER.PT = STEPMAXTIMEOUT;
    STEPMAXTIMER();
    STEPMAXTIMER.ENO = true;
    STEPMAXPREVSTATE = STATE;
    DRAININGTIMER.IN = (STATE == E_AUTOCYCLESTEP::AX13_DRAIN_PAUSE);
    DRAININGTIMER.PT = DRAINTIMEEFF;
    DRAININGTIMER();
    DRAININGTIMER.ENO = true;
    KOBOLDSTABTIMER.IN = ((((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION)) & (DEADMANARMED)) & (JOYSTICKPUSH)) & (KOBOLDIMMERSED);
    KOBOLDSTABTIMER.PT = CST_KOBOLDSTABTIME;
    KOBOLDSTABTIMER();
    KOBOLDSTABTIMER.ENO = true;
    BOTTOMTOUCHSTABTIMER.IN = (STATE == E_AUTOCYCLESTEP::AX8_BOTTOM_CONFIRMED);
    BOTTOMTOUCHSTABTIMER.PT = CST_BOTTOMTOUCHSTABTIME;
    BOTTOMTOUCHSTABTIMER();
    BOTTOMTOUCHSTABTIMER.ENO = true;
    RETRYJOYRELEASETIMER.IN = (((((STATE == E_AUTOCYCLESTEP::AX5_KOBOLD_INIT)) | ((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION))) | ((STATE == E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM)))) & (NOT((DEADMANARMED) & (JOYSTICKPUSH)));
    RETRYJOYRELEASETIMER.PT = 200000000LL;
    RETRYJOYRELEASETIMER();
    RETRYJOYRELEASETIMER.ENO = true;
    PALIERNOT4TIMER.IN = false;
    PALIERNOT4TIMER.PT = 5000000000LL;
    PALIERNOT4TIMER();
    PALIERNOT4TIMER.ENO = true;
    DIVINGRETRYTRIG = false;
    INSTCAUSES.at(0).ACTIVE = (LIMITLEGALREACHED) & ((((((STATE == E_AUTOCYCLESTEP::AX4_DESCEND_DIVING)) | ((STATE == E_AUTOCYCLESTEP::AX5_KOBOLD_INIT))) | ((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION))) | ((STATE == E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM))));
    INSTCAUSES.at(0).LATCHING = true;
    INSTCAUSES.at(0).TEXTE = "Limite legale de profondeur atteinte pendant descente";
    INSTCAUSES.at(1).ACTIVE = ((WINCHSYNCERROR) & ((STATE != E_AUTOCYCLESTEP::AX0_REPOS))) & ((STATE != E_AUTOCYCLESTEP::AX_STAB));
    INSTCAUSES.at(1).LATCHING = true;
    INSTCAUSES.at(1).TEXTE = "Desynchronisme treuils M1/M2 bloquant";
    INSTCAUSES.at(2).ACTIVE = ((SPEEDMISMATCHCONFIRMED) & ((STATE != E_AUTOCYCLESTEP::AX0_REPOS))) & ((STATE != E_AUTOCYCLESTEP::AX_STAB));
    INSTCAUSES.at(2).LATCHING = true;
    INSTCAUSES.at(2).TEXTE = "Ecart de vitesse M1/M2 confirme en montee";
    INSTCAUSES.at(3).ACTIVE = ((!HEARTBEATIHMOK) & ((STATE != E_AUTOCYCLESTEP::AX0_REPOS))) & ((STATE != E_AUTOCYCLESTEP::AX_STAB));
    INSTCAUSES.at(3).LATCHING = true;
    INSTCAUSES.at(3).TEXTE = "Perte communication IHM pendant cycle";
    INSTCAUSES.at(4).ACTIVE = (STEPMAXTIMER.Q) & ((STATE != E_AUTOCYCLESTEP::AX_STAB));
    INSTCAUSES.at(4).LATCHING = true;
    INSTCAUSES.at(4).TEXTE = "Depassement tempo maximale d etape (blocage mouvement)";
    INSTCAUSES.at(5).ACTIVE = (((STABILIZATIONTIMER.Q) | ((ABS(M1_CABLEPOSM - M2_CABLEPOSM) > CTRLASCENTTOLERANCEM)))) & ((STATE == E_AUTOCYCLESTEP::AX11_CTRL_ASCENT));
    INSTCAUSES.at(5).LATCHING = true;
    INSTCAUSES.at(5).TEXTE = "Defaut de stabilisation / tolerance ecart codeurs en remontee controle";
    INSTCAUSES.at(6).ACTIVE = (((ABS(WINCHSYNCDELTAM) > CST_COUPLEDPOSBACKLASHM)) & (JOYSTICKDEFLECTED)) & (((((((((STATE == E_AUTOCYCLESTEP::AX4_DESCEND_DIVING)) | ((STATE == E_AUTOCYCLESTEP::AX5_KOBOLD_INIT))) | ((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION))) | ((STATE == E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM))) | ((STATE == E_AUTOCYCLESTEP::AX9_RAISE_OFF_BOTTOM))) | ((STATE == E_AUTOCYCLESTEP::AX12_LOADED_ASCENT))) | ((STATE == E_AUTOCYCLESTEP::AX15C_DUMP_REPOSITION))));
    INSTCAUSES.at(6).LATCHING = true;
    INSTCAUSES.at(6).TEXTE = "Ecart de position M1/M2 excessif en phase couplee (anti-telescopage)";
    INSTCAUSES.at(7).ACTIVE = (!WINCHESATTOPWINDOW) & ((STATE == E_AUTOCYCLESTEP::AX2_TRANSLATE_P1));
    INSTCAUSES.at(7).LATCHING = true;
    INSTCAUSES.at(7).TEXTE = "Treuils M1/M2 hors FDC haut pendant la translation vers P1";
    INSTCAUSES.at(8).ACTIVE = (((((M1_SPEEDSTEPAPPLIED > CST_STEPDIVE)) | ((M2_SPEEDSTEPAPPLIED > CST_STEPDIVE))))) & ((((((STATE == E_AUTOCYCLESTEP::AX4_DESCEND_DIVING)) | ((STATE == E_AUTOCYCLESTEP::AX5_KOBOLD_INIT))) | ((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION))) | ((STATE == E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM))));
    INSTCAUSES.at(8).LATCHING = true;
    INSTCAUSES.at(8).TEXTE = "Palier vitesse > 4 (palier 5) pendant la plongee - interdit";
    WINCHESATDIVESTEP = ((M1_SPEEDSTEPAPPLIED == CST_STEPDIVE)) & ((M2_SPEEDSTEPAPPLIED == CST_STEPDIVE));
    INSTCAUSES.at(9).ACTIVE = (!WINCHESATDIVESTEP) & ((((STATE == E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION)) | ((STATE == E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM))));
    INSTCAUSES.at(9).LATCHING = true;
    INSTCAUSES.at(9).TEXTE = "Palier 4 non confirme sur M1 et/ou M2 pendant la recherche Kobold";
    INSTCAUSES.at(10).ACTIVE = (DIVESTARTTIMEOUTTIMER.Q) & ((STATE == E_AUTOCYCLESTEP::AX3_WAIT_DIVE_START));
    INSTCAUSES.at(10).LATCHING = true;
    INSTCAUSES.at(10).TEXTE = "Arret mecanique M1/M2 non confirme avant plongee";
    ...
    FORCESTEPAPPLYEDGE();
    FORCESTEPAPPLYEDGE.ENO = true;
    if ((FORCESTEPAPPLYEDGE.Q) & (CFGCOMMISSIONINGENABLE)) {
        if (((CFGFORCESTEPTARGET >= 0)) & ((CFGFORCESTEPTARGET <= 20))) {
            switch (CFGFORCESTEPTARGET) {
                case 0:
                    STATE = E_AUTOCYCLESTEP::AX0_REPOS;
                    break;
                case 1:
    ...
    if (RESETEDGE.Q) {
        ...
        BOTTOMTOUCHED = false;
        if (STATE == E_AUTOCYCLESTEP::AX_STAB) {
            STATE = E_AUTOCYCLESTEP::AX0_REPOS;
        }
    }
    if (((!ENABLE) | (!POWERCONTACTORENGAGED)) | (ENCODERFAULTPRESENT)) {
        WINCHM1CMD.RUNREQUEST = false;
        WINCHM1CMD.REQASCENT = false;
        WINCHM1CMD.REQDESCEND = false;
        WINCHM1CMD.STEPTGT = 0;
        ...
        BUCKETCMD.REQCLOSE = false;
        BUCKETCMD.REQKOBOLDMEASUREENABLE = false;
        if (((((STATE != E_AUTOCYCLESTEP::AX0_REPOS)) & ((STATE != E_AUTOCYCLESTEP::AX1_INIT))) & ((STATE != E_AUTOCYCLESTEP::AX18_DONE_SYNC))) & ((STATE != E_AUTOCYCLESTEP::AX_STAB))) {
            PAUSEDSTATE = STATE;
            WAITINGRESUME = true;
        } else {
            WAITINGRESUME = false;
    ...
    CYCLEMOTIONPERMIT = (JOYSTICKDEFLECTED) & (DEADMANARMED);
    if (WAITINGRESUME) {
        if (ABORTEDGE.Q) {
            STATE = E_AUTOCYCLESTEP::AX0_REPOS;
            WAITINGRESUME = false;
            PAUSEDSTATE = E_AUTOCYCLESTEP::AX0_REPOS;
        } else if ((STARTEDGE.Q) | (DEADMANARMEDEDGE.Q)) {
            STATE = PAUSEDSTATE;
            WAITINGRESUME = false;
        } else {
            WINCHM1CMD.RUNREQUEST = false;
    ...
    if ((DIVINGRETRYTRIG) & ((STATE != E_AUTOCYCLESTEP::AX_STAB))) {
        STATE = E_AUTOCYCLESTEP::AX_DIVING_RETRY;
    }
    ...
    if (!WAITINGRESUME) {
        switch (STATE) {
            case E_AUTOCYCLESTEP::AX0_REPOS:
                ...
                BOTTOMTOUCHED = false;
                if ((((STARTEDGE.Q) | (DEADMANARMEDEDGE.Q))) & (!FAULT.LATCHED)) {
                    WAITINGRESUME = false;
                    STATE = E_AUTOCYCLESTEP::AX1_INIT;
                }
                break;
            case E_AUTOCYCLESTEP::AX1_INIT:
                CYCLESTEP = E_AUTOCYCLESTEP::AX1_INIT;
                LIFECYCLE.BUSY = true;
                LIFECYCLE.DONE = false;
                EXPECTEDDIRECTION = 0;
                INITCONDITIONSOK = (((HOMEDM1) & (HOMEDM2)) & (!FAULT.LATCHED)) & (INITPOSITIONOK);
                if (INITCONDITIONSOK) {
                ...
                break;
            case E_AUTOCYCLESTEP::AX2_TRANSLATE_P1:
                ...
                WINCHM2CMD.REQDESCEND = false;
                WINCHM2CMD.STEPTGT = 0;
                TRANSLATIONCMD.POSITIONTGT = 3;
                if (TRANSLATION_AT_P1) {
                    EXPECTEDDIRECTION = 0;
                    WAITINGFOROPERATOR = false;
                    WAITINGFORPROCESS = true;
                    OPERATORACTIONID = 0x0402;
                    OPERATORACTION = "AX2 - Position P1 atteinte.";
                    TRANSLATIONCMD.REQSTART = false;
                    STATE = E_AUTOCYCLESTEP::AX3_OPEN_BUCKET;
                } else {
                    EXPECTEDDIRECTION = 1;
                ...
                break;
            case E_AUTOCYCLESTEP::AX3_OPEN_BUCKET:
                ...
                EXPECTEDDIRECTION = -1;
                WAITINGFOROPERATOR = NOT((DEADMANARMED) & (JOYSTICKPUSH));
                WAITINGFORPROCESS = (DEADMANARMED) & (JOYSTICKPUSH);
                ...
                WINCHM2CMD.REQDESCEND = false;
                WINCHM2CMD.STEPTGT = 0;
                if ((DEADMANARMED) & (JOYSTICKPUSH)) {
                    BUCKETCMD.REQOPEN = true;
                } else {
                    BUCKETCMD.REQOPEN = false;
                }
                if ((BENNE_ISOPEN) & (!BENNE_BUSY)) {
                    BUCKETCMD.REQOPEN = false;
                    STATE = E_AUTOCYCLESTEP::AX3_WAIT_DIVE_START;
                }
                break;
            case E_AUTOCYCLESTEP::AX3_WAIT_DIVE_START:
                ...
                EXPECTEDDIRECTION = -1;
                WAITINGFOROPERATOR = NOT((DEADMANARMED) & (JOYSTICKPUSH));
                WAITINGFORPROCESS = !DIVESTARTSTOPTIMER.Q;
                OPERATORACTIONID = 0x0403;
                ...
                BUCKETCMD.REQOPEN = false;
                BUCKETCMD.REQCLOSE = false;
                if (((DIVESTARTSTOPTIMER.Q) & (DEADMANARMED)) & (JOYSTICKPUSH)) {
                    STATE = E_AUTOCYCLESTEP::AX4_DESCEND_DIVING;
                }
                break;
            case E_AUTOCYCLESTEP::AX4_DESCEND_DIVING:
                ...
                WINCHM1CMD.RUNREQUEST = (DEADMANARMED) & (JOYSTICKPUSH);
                WINCHM1CMD.REQASCENT = false;
                WINCHM1CMD.REQDESCEND = true;
                WINCHM1CMD.STEPTGT = CST_STEPDIVE;
                WINCHM2CMD.RUNREQUEST = (DEADMANARMED) & (JOYSTICKPUSH);
                WINCHM2CMD.REQASCENT = false;
                WINCHM2CMD.REQDESCEND = true;
                WINCHM2CMD.STEPTGT = CST_STEPDIVE;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = false;
                if (((DEADMANARMED) & (JOYSTICKPUSH)) & ((M1_CABLEPOSM <= DIVESTARTMINEFF))) {
                    STATE = E_AUTOCYCLESTEP::AX5_KOBOLD_INIT;
                }
                break;
            case E_AUTOCYCLESTEP::AX5_KOBOLD_INIT:
                ...
                WINCHM2CMD.REQDESCEND = true;
                WINCHM2CMD.STEPTGT = CST_STEPDIVE;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = (DEADMANARMED) & (JOYSTICKPUSH);
                if (((DEADMANARMED) & (JOYSTICKPUSH)) & (KOBOLDCONTACTORFEEDBACK)) {
                    STATE = E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION;
                }
                break;
            case E_AUTOCYCLESTEP::AX6_SEARCH_IMMERSION:
                ...
                WINCHM2CMD.STEPTGT = CST_STEPDIVE;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = (DEADMANARMED) & (JOYSTICKPUSH);
                if (((DEADMANARMED) & (JOYSTICKPUSH)) & (KOBOLDSTABTIMER.Q)) {
                    STATE = E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM;
                }
                break;
            case E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM:
                CYCLESTEP = E_AUTOCYCLESTEP::AX7_SEARCH_BOTTOM;
                CYCLESTATESTR = "AX7 - Recherche du fond (front descendant DI Kobold 1->0)";
                LIFECYCLE.BUSY = true;
                LIFECYCLE.DONE = false;
                EXPECTEDAXIS = E_OPERATORAXIS::JOYSTICK_Y;
                EXPECTEDDIRECTION = -1;
                ...
                WINCHM2CMD.STEPTGT = CST_STEPDIVE;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = (DEADMANARMED) & (JOYSTICKPUSH);
                if (((DEADMANARMED) & (JOYSTICKPUSH)) & (((KOBOLDFALLEDGE.Q) | (!KOBOLDIMMERSED)))) {
                    TOUCHPOSITIONM = M1_CABLEPOSM;
                    BOTTOMTOUCHED = true;
                    if (LIMITLEGALDEPTHM < -0.01) {
                        RAISETARGETM = MAX(TOUCHPOSITIONM + RAISEOFFBOTTOMEFF, LIMITLEGALDEPTHM + RAISEOFFBOTTOMEFF);
                    } else {
                        RAISETARGETM = TOUCHPOSITIONM + RAISEOFFBOTTOMEFF;
                    }
                    STATE = E_AUTOCYCLESTEP::AX8_BOTTOM_CONFIRMED;
                }
                break;
            case E_AUTOCYCLESTEP::AX8_BOTTOM_CONFIRMED:
                CYCLESTEP = E_AUTOCYCLESTEP::AX8_BOTTOM_CONFIRMED;
                CYCLESTATESTR = "AX8 - Fond confirme : arret treuils et stabilisation (2s)";
                LIFECYCLE.BUSY = true;
                LIFECYCLE.DONE = false;
                EXPECTEDAXIS = E_OPERATORAXIS::NONE;
                EXPECTEDDIRECTION = 0;
                ...
                OPERATORACTION = "AX8 - Fond touche : stabilisation et arret des treuils.";
                WINCHM1CMD.RUNREQUEST = false;
                WINCHM1CMD.REQASCENT = false;
                WINCHM1CMD.REQDESCEND = false;
                WINCHM1CMD.STEPTGT = 0;
                ...
                WINCHM2CMD.STEPTGT = 0;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = false;
                if (BOTTOMTOUCHSTABTIMER.Q) {
                    STATE = E_AUTOCYCLESTEP::AX9_RAISE_OFF_BOTTOM;
                }
                break;
            case E_AUTOCYCLESTEP::AX_DIVING_RETRY:
                ...
                WINCHM1CMD.REQASCENT = true;
                WINCHM1CMD.REQDESCEND = false;
                WINCHM1CMD.STEPTGT = CST_STEPSLOW;
                WINCHM2CMD.RUNREQUEST = CYCLEMOTIONPERMIT;
                WINCHM2CMD.REQASCENT = true;
                WINCHM2CMD.REQDESCEND = false;
                WINCHM2CMD.STEPTGT = CST_STEPSLOW;
                BUCKETCMD.REQKOBOLDMEASUREENABLE = false;
                if ((JOYSTICKDEFLECTED) & (WINCHESATTOPWINDOW)) {
                    WINCHM1CMD.RUNREQUEST = false;
                    WINCHM2CMD.RUNREQUEST = false;
                    STATE = E_AUTOCYCLESTEP::AX4_DESCEND_DIVING;
                }
                break;
            case E_AUTOCYCLESTEP::AX9_RAISE_OFF_BOTTOM:
                CYCLESTEP = E_AUTOCYCLESTEP::AX9_RAISE_OFF_BOTTOM;
                CYCLESTATESTR = "AX9 - Retente les cables (palier 1), decollement du fond";
                LIFECYCLE.BUSY = true;
                LIFECYCLE.DONE = false;
                EXPECTEDAXIS = E_OPERATORAXIS::JOYSTICK_Y;
                EXPECTEDDIRECTION = 1;
                ...
                OPERATORACTION = "AX9 - Maintenir la montee palier 1 : retente des cables, decollement du fond.";
                WINCHM1CMD.RUNREQUEST = CYCLEMOTIONPERMIT;
                WINCHM1CMD.REQASCENT = true;
                WINCHM1CMD.REQDESCEND = false;
                WINCHM1CMD.STEPTGT = CST_STEPSLOW;
                ...
                WINCHM2CMD.REQDESCEND = false;
                WINCHM2CMD.STEPTGT = CST_STEPSLOW;
                if (((JOYSTICKDEFLECTED) & ((M1_CABLEPOSM >= RAISETARGETM))) & ((M2_CABLEPOSM >= RAISETARGETM))) {
                    WINCHM1CMD.RUNREQUEST = false;
                    WINCHM2CMD.RUNREQUEST = false;
                    STATE = E_AUTOCYCLESTEP::AX10_CLOSE_BUCKET;
                }
                break;
            case E_AUTOCYCLESTEP::AX10_CLOSE_BUCKET:
                ...
                if ((JOYSTICKDEFLECTED) & (((BENNE_DONE) & (((BENNE_ISCLOSED) | (BENNE_ISROUGHLYCLOSED)))))) {
                    BUCKETCMD.REQCLOSE = false;
                    WINCHM1CMD.RUNREQUEST = CYCLEMOTIONPERMIT;
                    WINCHM1CMD.REQASCENT = true;
                    WINCHM1CMD.REQDESCEND = false;
                ...
                break;
            case E_AUTOCYCLESTEP::AX11_CTRL_ASCENT:
                CYCLESTEP = E_AUTOCYCLESTEP::AX11_CTRL_ASCENT;
                CYCLESTATESTR = "AX11 - Remontee lente de controle";
                LIFECYCLE.BUSY = true;
                LIFECYCLE.DONE = false;
                EXPECTEDAXIS = E_OPERATORAXIS::JOYSTICK_Y;
                EXPECTEDDIRECTION = 1;
                ...
                OPERATORACTION = "AX11 - Maintenir la montee de controle : palier et synchronisme.";
                WINCHM1CMD.RUNREQUEST = CYCLEMOTIONPERMIT;
                WINCHM1CMD.REQASCENT = true;
                WINCHM1CMD.REQDESCEND = false;
                WINCHM1CMD.STEPTGT = CTRLASCENTMAXSTEPEFF;
                ...
                WINCHM2CMD.REQDESCEND = false;
                WINCHM2CMD.STEPTGT = CTRLASCENTMAXSTEPEFF;
                if ((((JOYSTICKDEFLECTED) & ((M1_CABLEPOSM >= (TOUCHPOSITIONM + CTRLASCENTDISTM)))) & ((M2_CABLEPOSM >= (TOUCHPOSITIONM + CTRLASCENTDISTM)))) & ((ABS(M1_CABLEPOSM - M2_CABLEPOSM) <= CTRLASCENTTOLERANCEM))) {
                    STATE = E_AUTOCYCLESTEP::AX12_LOADED_ASCENT;
                }
                break;
            case E_AUTOCYCLESTEP::AX12_LOADED_ASCENT:
                ...
                if ((JOYSTICKDEFLECTED) & (M1_CABLEPOSM >= CABLELIMITM1ASCENTM)) {
                    WINCHM1CMD.RUNREQUEST = false;
                    WINCHM2CMD.RUNREQUEST = false;
                    STATE = E_AUTOCYCLESTEP::AX13_DRAIN_PAUSE;
                }
                break;
            case E_AUTOCYCLESTEP::AX13_DRAIN_PAUSE:
                ...
                WINCHM1CMD.RUNREQUEST = false;
                WINCHM2CMD.RUNREQUEST = false;
                if ((JOYSTICKDEFLECTED) & (((DRAININGTIMER.Q) | (SKIPDRAINEDGE.Q)))) {
                    STATE = E_AUTOCYCLESTEP::AX14_TRANSLATE_DUMP;
                }
                break;
            case E_AUTOCYCLESTEP::AX14_TRANSLATE_DUMP:
                ...
                TRANSLATIONCMD.POSITIONTGT = 1;
                TRANSLATIONCMD.REQSTART = CYCLEMOTIONPERMIT;
                if ((JOYSTICKDEFLECTED) & (TRANSLATION_AT_TREMIE)) {
                    TRANSLATIONCMD.REQSTART = false;
                    STATE = E_AUTOCYCLESTEP::AX15A_DUMP_ARRIVE;
                }
                break;
            case E_AUTOCYCLESTEP::AX15A_DUMP_ARRIVE:
                ...
                break;
            case E_AUTOCYCLESTEP::AX15B_DUMP_OPEN:
                ...
                if (REPOSITIONREQUEST) {
                    STATE = E_AUTOCYCLESTEP::AX15C_DUMP_REPOSITION;
                } else if (((JOYSTICKDEFLECTED) & (BENNE_DONE)) & (BENNE_ISOPEN)) {
                    BUCKETCMD.REQOPEN = false;
                    STATE = E_AUTOCYCLESTEP::AX18_DONE_SYNC;
                }
                break;
            case E_AUTOCYCLESTEP::AX15C_DUMP_REPOSITION:
                ...
                WINCHM2CMD.RUNREQUEST = CYCLEMOTIONPERMIT;
                WINCHM2CMD.STEPTGT = CST_STEPSLOW;
                BUCKETCMD.REQOPEN = (CYCLEMOTIONPERMIT) & (!BENNE_ISOPEN);
                if (!REPOSITIONREQUEST) {
                    WINCHM1CMD.RUNREQUEST = false;
                    WINCHM2CMD.RUNREQUEST = false;
                    STATE = E_AUTOCYCLESTEP::AX15B_DUMP_OPEN;
                ...
                break;
            case E_AUTOCYCLESTEP::AX18_DONE_SYNC:
                ...
                if (!SAMPLECOUNTDONE) {
                    ...
                    LASTCYCLEDURATION = CURRENTCYCLEELAPSED;
                    SAMPLECOUNTDONE = true;
                }
                if ((STARTEDGE.Q) | (JOYDEFLECTEDEDGE.Q)) {
                    SAMPLECOUNTDONE = false;
                    LIFECYCLE.DONE = false;
                    STATE = E_AUTOCYCLESTEP::AX2_TRANSLATE_P1;
                }
                break;
            case E_AUTOCYCLESTEP::AX_STAB:
                CYCLESTEP = E_AUTOCYCLESTEP::AX_STAB;
                CYCLESTATESTR = "AX_STAB - Cycle en attente / stabilisation sur anomalie";
                WINCHM1CMD.RUNREQUEST = false;
                WINCHM1CMD.REQASCENT = false;
                WINCHM1CMD.REQDESCEND = false;
                WINCHM2CMD.RUNREQUEST = false;
                ...
                OPERATORACTION = "AX_STAB - Cycle stabilise sur anomalie : acquitter le defaut pour reinitialiser.";
                break;
        }
    }
    ...
    REQUESTACTIVE = (((((WINCHM1CMD.RUNREQUEST) | (WINCHM2CMD.RUNREQUEST)) | (TRANSLATIONCMD.REQSTART)) | (BUCKETCMD.REQOPEN)) | (BUCKETCMD.REQCLOSE)) | (BUCKETCMD.REQKOBOLDMEASUREENABLE);
}
