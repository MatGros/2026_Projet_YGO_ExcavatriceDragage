...
FB_SAFETY_EMERGENCYMANAGEMENT::FB_SAFETY_EMERGENCYMANAGEMENT()
    : REDUNDANCYTESTFAILEDACK(true), EMERGENCYARMINGFAILEDACK(true), CST_TESTDURATION(200000000LL), CST_ARMINGPULSEDURATION(1000000000LL), CST_ARMINGCONFIRMTIMEOUT(2000000000LL), CST_ARMINGLOCKOUT(5000000000LL), CST_FAULTDISPLAYDEBOUNCE(200000000LL)
{
    // Initialize variables
}

void FB_SAFETY_EMERGENCYMANAGEMENT::operator()() {
    if (__mocked_) { __mock_state_.call_count++; return; }
    ENABLERISING.CLK = ENABLE;
    ENABLERISING();
    ENABLERISING.ENO = true;
    RESETEDGE.CLK = RESET;
    RESETEDGE();
    RESETEDGE.ENO = true;
    ...
    if (!ENABLE) {
        ...
        LASTABORTCAUSE = TO_WORD(CST_ABORT_NONE);
        ...
        TONRESTOREA();
        TONRESTOREA.ENO = true;
        TONTESTB.IN = false;
        TONTESTB();
        TONTESTB.ENO = true;
        TONRESTOREB.IN = false;
        TONRESTOREB();
        TONRESTOREB.ENO = true;
        TONARMINGPULSE.IN = false;
        TONARMINGPULSE();
        TONARMINGPULSE.ENO = true;
        TONARMINGCONFIRM.IN = false;
        TONARMINGCONFIRM();
        TONARMINGCONFIRM.ENO = true;
        TONARMINGLOCKOUT.IN = false;
        TONARMINGLOCKOUT();
        TONARMINGLOCKOUT.ENO = true;
        TONREDUNDANCYDISPLAY.IN = false;
        TONREDUNDANCYDISPLAY();
        TONREDUNDANCYDISPLAY.ENO = true;
        TONARMINGFAILEDDISPLAY.IN = false;
        TONARMINGFAILEDDISPLAY();
        TONARMINGFAILEDDISPLAY.ENO = true;
    }
    if (ENABLE) {
        if (ENABLERISING.Q) {
            STARTUPCHECKDONE = false;
        ...
        if (!STARTUPCHECKDONE) {
            STARTUPCHECKOK = ((EMERGENCYCHAINCLOSED) & (!POWERCONTACTORENGAGED)) & ((ARMINGSEQSTEP == CST_STEP_IDLE));
            STARTUPCHECKDONE = true;
            STARTUPFAIL = !STARTUPCHECKOK;
        }
    }
    if (ENABLE) {
        if (RESETEDGE.Q) {
    ...
    if (ENABLE) {
        if (ARMINGSEQSTEP != CST_STEP_IDLE) {
            if (BTNEMERGENCYCUTOFF) {
                LASTABORTSTEP = ARMINGSEQSTEP;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_CUT_IHM);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            } else if (POWERCUTOFFREQUEST) {
                EMERGENCYARMINGLOCKOUTACTIVE = true;
                LASTABORTSTEP = ARMINGSEQSTEP;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_POWER_CUTOFF);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            } else if ((((ARMINGSEQSTEP == CST_STEP_PULSE) | (ARMINGSEQSTEP == CST_STEP_CONFIRM))) & (!EMERGENCYCHAINCLOSED)) {
                EMERGENCYARMINGFAILEDCAUSE = true;
                EMERGENCYARMINGLOCKOUTACTIVE = true;
                LASTABORTSTEP = ARMINGSEQSTEP;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_CHAIN_DROP);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            }
        }
        ARMABLE_CHAINOK = ((EMERGENCYCHAINCLOSED) & (!BTNEMERGENCYCUTOFF)) & (!POWERCUTOFFREQUEST);
        ARMABLE_IDLE = (ARMINGSEQSTEP == CST_STEP_IDLE);
        ARMABLE_NOBLOCK = (!EMERGENCYARMINGLOCKOUTACTIVE) & (!POWERCONTACTORENGAGED);
        ARMABLE = ((ARMABLE_CHAINOK) & (ARMABLE_IDLE)) & (ARMABLE_NOBLOCK);
        if ((ARMREQEDGE.Q) & (ARMABLE)) {
            ARMINGSEQSTEP = CST_STEP_TEST_A;
            REDUNDANCYTESTFAILEDCAUSE = false;
            EMERGENCYARMINGFAILEDCAUSE = false;
        }
        TONTESTA.IN = (ARMINGSEQSTEP == CST_STEP_TEST_A);
        TONTESTA.PT = CST_TESTDURATION;
        TONTESTA();
        TONTESTA.ENO = true;
        if (ARMINGSEQSTEP == CST_STEP_TEST_A) {
            FORCETESTA = true;
            if (TONTESTA.Q) {
                FORCETESTA = false;
                if (EMERGENCYCHAINCLOSED) {
                    ...
                    LASTABORTCAUSE = TO_WORD(CST_ABORT_REDUNDANCY_A);
                    ARMINGSEQSTEP = CST_STEP_IDLE;
                } else {
        ...
        TONRESTOREA.IN = (ARMINGSEQSTEP == CST_STEP_RESTORE_A);
        TONRESTOREA.PT = CST_TESTDURATION;
        TONRESTOREA();
        TONRESTOREA.ENO = true;
        if (((ARMINGSEQSTEP == CST_STEP_RESTORE_A)) & (TONRESTOREA.Q)) {
            if (EMERGENCYCHAINCLOSED) {
                ARMINGSEQSTEP = CST_STEP_TEST_B;
            } else {
                LASTABORTSTEP = CST_STEP_RESTORE_A;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_TIMEOUT_RESTORE_A);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            }
        }
        TONTESTB.IN = (ARMINGSEQSTEP == CST_STEP_TEST_B);
        TONTESTB.PT = CST_TESTDURATION;
        TONTESTB();
        TONTESTB.ENO = true;
        if (ARMINGSEQSTEP == CST_STEP_TEST_B) {
            FORCETESTB = true;
            if (TONTESTB.Q) {
                FORCETESTB = false;
                if (EMERGENCYCHAINCLOSED) {
                    ...
                    LASTABORTCAUSE = TO_WORD(CST_ABORT_REDUNDANCY_B);
                    ARMINGSEQSTEP = CST_STEP_IDLE;
                } else {
        ...
        TONRESTOREB.IN = (ARMINGSEQSTEP == CST_STEP_RESTORE_B);
        TONRESTOREB.PT = CST_TESTDURATION;
        TONRESTOREB();
        TONRESTOREB.ENO = true;
        if (((ARMINGSEQSTEP == CST_STEP_RESTORE_B)) & (TONRESTOREB.Q)) {
            if (EMERGENCYCHAINCLOSED) {
                ARMINGSEQSTEP = CST_STEP_PULSE;
            } else {
                LASTABORTSTEP = CST_STEP_RESTORE_B;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_TIMEOUT_RESTORE_B);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            }
        }
        TONARMINGPULSE.IN = (ARMINGSEQSTEP == CST_STEP_PULSE);
        TONARMINGPULSE.PT = CST_ARMINGPULSEDURATION;
        TONARMINGPULSE();
        TONARMINGPULSE.ENO = true;
        if (((ARMINGSEQSTEP == CST_STEP_PULSE)) & (TONARMINGPULSE.Q)) {
            ARMINGSEQSTEP = CST_STEP_CONFIRM;
        }
        TONARMINGCONFIRM.IN = (ARMINGSEQSTEP == CST_STEP_CONFIRM);
        TONARMINGCONFIRM.PT = CST_ARMINGCONFIRMTIMEOUT;
        TONARMINGCONFIRM();
        TONARMINGCONFIRM.ENO = true;
        if (ARMINGSEQSTEP == CST_STEP_CONFIRM) {
            if (POWERCONTACTORENGAGED) {
                ARMINGSEQSTEP = CST_STEP_IDLE;
                EMERGENCYARMINGLOCKOUTACTIVE = false;
            } else if (TONARMINGCONFIRM.Q) {
                EMERGENCYARMINGFAILEDCAUSE = true;
                EMERGENCYARMINGLOCKOUTACTIVE = true;
                LASTABORTSTEP = CST_STEP_CONFIRM;
                LASTABORTCAUSE = TO_WORD(CST_ABORT_TIMEOUT_CONTACTOR);
                ARMINGSEQSTEP = CST_STEP_IDLE;
            }
        }
        TONARMINGLOCKOUT.IN = EMERGENCYARMINGLOCKOUTACTIVE;
        TONARMINGLOCKOUT.PT = CST_ARMINGLOCKOUT;
        TONARMINGLOCKOUT();
        TONARMINGLOCKOUT.ENO = true;
        if (TONARMINGLOCKOUT.Q) {
            EMERGENCYARMINGLOCKOUTACTIVE = false;
        }
    }
    if (ENABLE) {
        ...
        REDUNDANCYCAUSEEDGE();
        REDUNDANCYCAUSEEDGE.ENO = true;
        if (REDUNDANCYCAUSEEDGE.Q) {
            REDUNDANCYTESTFAILEDACK = false;
        }
        ARMINGFAILEDCAUSEEDGE.CLK = EMERGENCYARMINGFAILEDCAUSE;
        ARMINGFAILEDCAUSEEDGE();
        ARMINGFAILEDCAUSEEDGE.ENO = true;
        if (ARMINGFAILEDCAUSEEDGE.Q) {
            EMERGENCYARMINGFAILEDACK = false;
        }
    }
    if (ENABLE) {
        ARMINGBUSY = ((ARMINGSEQSTEP != CST_STEP_IDLE)) | (EMERGENCYARMINGLOCKOUTACTIVE);
    }
    if (ENABLE) {
        TONREDUNDANCYDISPLAY.IN = REDUNDANCYTESTFAILEDCAUSE;
        TONREDUNDANCYDISPLAY.PT = CST_FAULTDISPLAYDEBOUNCE;
        TONREDUNDANCYDISPLAY();
        TONREDUNDANCYDISPLAY.ENO = true;
        REDUNDANCYTESTFAILED = (TONREDUNDANCYDISPLAY.Q) | (!REDUNDANCYTESTFAILEDACK);
        TONARMINGFAILEDDISPLAY.IN = EMERGENCYARMINGFAILEDCAUSE;
        TONARMINGFAILEDDISPLAY.PT = CST_FAULTDISPLAYDEBOUNCE;
        TONARMINGFAILEDDISPLAY();
        TONARMINGFAILEDDISPLAY.ENO = true;
        EMERGENCYARMINGFAILED = (TONARMINGFAILEDDISPLAY.Q) | (!EMERGENCYARMINGFAILEDACK);
    }
    INSTCAUSES.at(0).ACTIVE = REDUNDANCYTESTFAILED;
    INSTCAUSES.at(0).LATCHING = true;
    INSTCAUSES.at(0).TEXTE = "Defaut redondance contacteurs AU";
    INSTCAUSES.at(1).ACTIVE = EMERGENCYARMINGFAILED;
    INSTCAUSES.at(1).LATCHING = true;
    INSTCAUSES.at(1).TEXTE = "Echec rearmement contacteur AU (timeout)";
    INSTCAUSES.at(2).ACTIVE = false;
    INSTCAUSES.at(2).LATCHING = false;
    INSTCAUSES.at(2).TEXTE = "";
    INSTCAUSES.at(3).ACTIVE = STARTUPFAIL;
    INSTCAUSES.at(3).LATCHING = true;
    INSTCAUSES.at(3).TEXTE = "Echec autotest demarrage AU";
    INSTFAULT.ENABLE = ENABLE;
    INSTFAULT.RESET = RESET;
    INSTFAULT.CAUSES = INSTCAUSES;
    INSTFAULT();
    INSTFAULT.ENO = true;
    STATUS.FAULT = INSTFAULT.FAULT;
    MAINTAIN_CHAINOK = ((EMERGENCYCHAINCLOSED) | ((ARMINGSEQSTEP != CST_STEP_IDLE)));
    MAINTAIN_NOCUTOFF = (!POWERCUTOFFREQUEST) & (!BTNEMERGENCYCUTOFF);
    MAINTAINA_NOFAULT = (!FORCETESTA) & (!REDUNDANCYTESTFAILEDCAUSE);
    MAINTAINB_NOFAULT = (!FORCETESTB) & (!REDUNDANCYTESTFAILEDCAUSE);
    MAINTAINA_NOBLOCK = (MAINTAIN_NOCUTOFF) & (MAINTAINA_NOFAULT);
    MAINTAINB_NOBLOCK = (MAINTAIN_NOCUTOFF) & (MAINTAINB_NOFAULT);
    CMD.MAINTAINA_CMD = ((ENABLE) & (MAINTAIN_CHAINOK)) & (MAINTAINA_NOBLOCK);
    CMD.MAINTAINB_CMD = ((ENABLE) & (MAINTAIN_CHAINOK)) & (MAINTAINB_NOBLOCK);
    CMD.ARMPULSE_CMD = (((ENABLE) & ((ARMINGSEQSTEP == CST_STEP_PULSE))) & (!BTNEMERGENCYCUTOFF)) & (!POWERCUTOFFREQUEST);
    CMD.TESTCUTACTIVE = (ENABLE) & (((FORCETESTA) | (FORCETESTB)));
    STATUS.STATE.CHAINOK = (ENABLE) & (EMERGENCYCHAINCLOSED);
    STATUS.STATE.CONTACTOROK = (ENABLE) & (POWERCONTACTORENGAGED);
    ...
    STATUS.READY = (ENABLE) & (!STARTUPFAIL);
