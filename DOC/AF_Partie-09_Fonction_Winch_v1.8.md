# 📋 Analyse Fonctionnelle — Partie 9 : Fonction Winch (v1.8)

> 🆕 **v1.8 (2026-07-08)** — Lot TBD : Ajout note d'investigation sur montée en charge et temporisation frein (§4undecies).
...
> 🔗 Dépend de : [P2 Architecture v2.10](AF_Partie-02_Architecture_Programme_v2.10.md), [P3 Contrat FB v1.3](AF_Partie-03_Template_FB_Commun_v1.3.md), [P4 Cycle v1.2](AF_Partie-04_Cycle_Sequenceur_v1.2.md) §3bis/§4, [P5 Modes v1.3](AF_Partie-05_Modes_Maintenance_v1.3.md), [P8 Joystick v1.2](AF_Partie-08_Fonction_Joystick_v1.2.md).
>
...
- **Arrêt virtuel normal haut** : En fonctionnement normal, dès qu'un treuil est référencé (`Homed = TRUE` et `HomingSuspect = FALSE`), une limite virtuelle haute normale est activée. Pour le treuil M1 (holding), elle est à `HomingTargetM1_M - WinchTopStopMarginM` (par défaut 12.00m). Pour le treuil M2 (closing), afin de permettre la fermeture complète du grappin y compris à hauteur maximale, cette limite est décalée de l'offset de fermeture et se situe à `HomingTargetM2_M + OffsetCloseM - WinchTopStopMarginM` (par défaut 14.00m si `OffsetCloseM = 2.0m`). Dès que cette position est atteinte et si `HomingApproachEnable = FALSE`, la montée est interdite (`ForbidAscent := TRUE`), ce qui applique une rampe de décélération normale vers 0.
- **Limite logicielle absolue** : Dès que le treuil est référencé (`Homed = TRUE` et `HomingSuspect = FALSE`), une limite logicielle absolue est active. Pour le treuil M1, elle est à `HomingTargetM1_M` (12.50m). Pour le treuil M2, elle est décalée de l'offset de fermeture et se situe à `HomingTargetM2_M + OffsetCloseM` (par défaut 14.50m si `OffsetCloseM = 2.0m`). Dès que cette limite absolue est dépassée, la montée est inconditionnellement interdite (`ForbidAscent := TRUE`).
...
### 🆕 4undecies. Investigations futures — Montée en charge et temporisation frein (TBD, v1.8)
...
**Point de vigilance** : La séquence de frein (relâche après magnétisation du moteur, collage
après décélération/ouverture des contacteurs) s'appuie sur des temporisations fixes
(`DelayMagnetise`, `DelayMotorDecel`). **En montée chargée**, il faut valider que :
- Le frein relâché assez tôt (délai magnétisation) pour transférer la charge au moteur sans
  à-coup brutal.
- Le frein collé assez tard (délai décélération) pour que le moteur ait bien arrêté sa rotation
  avant serrage (sinon usure/casse mécanique).
- Aucun "rebondissement" ou "glissement frein" induit par la charge entraînante durant la phase
  de transition.

**Refactoring demandé** : À définir lors d'un lot dédié — validation terrain + éventuels réglages
de temporisations, voire refonte du modèle séquentiel du frein selon le retour d'expérience mise
en service. **Pas d'action immédiate** ; point à tracker pour essais de charge.
