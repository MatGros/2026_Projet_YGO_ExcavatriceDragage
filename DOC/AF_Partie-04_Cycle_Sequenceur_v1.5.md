# 📋 Analyse Fonctionnelle — Partie 4 : Cycle & Séquenceur (v1.5)

> **v1.5 — Assistants prélèvement maintenance (2026-07-28)** : `FB_DiveSearch` et
> `FB_ExtractionSequence` sont implémentés d'abord pour `MAINT_N1`/`MAINT_N2`, sans modifier
> l'orchestration `FB_Cycle`/`SEMI_AUTO`. Les deux reçoivent une intention opérateur déjà arbitrée
> (joystick ou boutons IHM), jamais une lecture directe de source. `FB_DiveSearch` valide la
> séquence Kobold `0→1→0`; `FB_ExtractionSequence` ferme via `FB_Bucket`, puis remonte M1/M2 au
> palier 1 (aucun contacteur vitesse) pendant `ExtractionControlDistance_M` avant l'accélération.
> Une attestation IHM temporaire `TglBucketAtBottomConfirmed` autorise l'extraction seule ; elle est
> consommée au départ extraction et annulée par descente, ouverture, reset, perte AU ou sortie maintenance.
> La consigne de conduite est unique : `GVL_IHM.OperatorAction.ActionId` + `.Text`; les alarmes restent
> dans `Error`/`ErrorId`.
>
...
3. **Sélection des Options en Maintenance** :
   - Les cases `TglEnableDiveSearch` et `TglEnableExtractionSequence` autorisent les briques,
     elles ne déclenchent jamais un mouvement. Une intention de descente/montée est toujours requise.
   - L'intention est unique et déjà arbitrée : joystick ou boutons IHM. Les FB ne connaissent pas
     la source et ne font pas de `OR` de commandes.
   - La case `TglBucketAtBottomConfirmed` est une attestation manuelle de position, pas un retour
     Kobold. Elle sert seulement à l'extraction seule.
