# 📋 Analyse Fonctionnelle — Partie 2 : Architecture Programme (v2.11)
...
| 5 | **`SafeStop` = 1 par bloc safety métier**, pas de signal global | `PRG_3_Safety` centralise les **instances** (`FB_Safety_Winch` ×2, `FB_Safety_Translation`), chacune garde son `SafeStop` propre |
...
| **M3** | `FB_Translation` (dans `PRG_7_TranslationControl`) | Variateur **AC600** axe transversal | EtherCAT |
...
(`FB_Safety_Winch`, défini dans `CODE/WINCH/`), `instSafetyTranslationM3` (`FB_Safety_Translation`,
...
### `PRG_7_TranslationControl` (position 7)
Arbitre la source de commande pour M3 (Joystick vs `PRG_5_Cycle`), sélectionne le capteur de
position cible actif (`PosFosse1/2`, `PosMaintenance`, `PosTremie`), instancie `FB_Translation`
...
| `GVL_Translation_M3_Stub` (`TRANSLATION`) | Stub sélecteur position test + signaux M3 non câblés | ✅ Conservée — même statut |
...
| ~~`GVL_Machine_Stub`~~ (`SYSTEM`) | Ancien porteur de `MachineReset_IHM` | ❌ **Supprimé (2026-07-15)** — `MachineReset_IHM` est calculé et exposé par `PRG_9_Supervision.FaultMachineReset_IHM` |
...
| Logicielle | `SafeStop` (sortie d'un bloc safety **métier**, 1 par domaine) | Variable interne | Rampe de décélération rapide (`Enable` maintenu). Calculée dans `PRG_3_Safety`, consommée directement par `FB_Winch`/`FB_Translation` dans `PRG_6`/`PRG_7`. |
