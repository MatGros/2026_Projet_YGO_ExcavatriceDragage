# 📋 Analyse Fonctionnelle — Partie 3 : Template FB Commun (v1.1)
...
## 🔌 7bis. Sécurité électrique — automate jamais coupé (spécifique `FB_Safety`)

⚠️ Le contrôleur **reste alimenté en permanence** : la sécurité ne peut pas reposer sur l'extinction de l'API.

Règle pour `FB_Safety` (détail fonctionnel en Partie 2) :
- 🔁 Chaque contacteur de puissance a un **retour d'état câblé** (entrée TOR).
- 🔍 `FB_Safety` compare en continu **commande vs retour** (`ST_ContactorCheck`).
...
- 🧭 Cette coupure est **indépendante** de `SafeStop` logiciel : c'est le dernier rempart matériel.
