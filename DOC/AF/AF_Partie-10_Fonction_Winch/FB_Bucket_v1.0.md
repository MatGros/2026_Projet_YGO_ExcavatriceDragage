...
<table style="width: 100%; table-layout: fixed; border-collapse: collapse; font-size: 14px;">
  ...
  <tbody>
    ...
    <tr style="border-bottom: 1px solid rgba(255,255,255,0.08);">
      ...
      <td style="padding: 6px 8px; line-height: 1.55;">
        💤 <b>Étape 0</b> : Manœuvre benne engagée (<code>Busy</code>) — la limite de recul s'appuie sur la <b>frontière physique</b> de la benne, jamais sur la position de départ de manœuvre<br>
        🚀 <b>Étape 1</b> : Recul (sens inverse : <code>ReqDescend</code> pendant une fermeture) sous commande toujours engagée<br>
        ⚡ <b>Étape 2</b> : Sens inverse honoré (permis opposé requis, <code>FB_Bucket.st:527</code>) : <code>M2_ReqDescend=TRUE</code>, <code>M2_ReqAscent=FALSE</code>, palier P1<br>
        ✅ <b>Étape 3</b> : Recul commandé <b>et borné</b> — le recul ne franchit pas la frontière physique (<code>Delta &lt;= OffsetOpenM - CoherenceLimitM</code> en fermeture, <code>Delta &gt;= OffsetCloseM + CoherenceLimitM</code> en ouverture), avec confirmation <code>CST_BandStableTime</code> ; seule la direction de recul est inhibée, le sens vers la cible reste servi et <code>Lifecycle</code> est intact (<code>TC-P10-029.1</code>, <code>TC-P10-T346-N1</code>)
      </td>
      <td style="padding: 4px 1px; text-align: center;"><small><code>💻 AUTO</code></small></td>
      <td style="padding: 4px 1px; text-align: center;"><small>§2</small></td>
      <td style="padding: 4px 1px; text-align: center;"><small><code>NV-I</code></small></td>
...
| Entrée | Type | Sens |
|---|---|---|
| `ReqAscent` / `ReqDescend` | BOOL | Demande de sens **déjà arbitrée** (joystick/IHM, axe Y) — `MotionRequestActive = (ReqAscent OR ReqDescend) AND NOT conflit` est calculé **en interne** (`FB_Bucket.st:160-161`) et **n'est PAS une entrée** : le passer en argument nommé est un argument MORT (garde-fou `G512`) |
| `EffectivePermitBucket_Open` / `_Close` | BOOL | Permis de sécurité : **sans eux, aucun ordre M2 ne sort** (`:523`, `:539`) |
...
| `M1_Busy`/`M2_Busy` | BOOL | Anti-traversée : refuse une **nouvelle** demande (`:481`, `:485`) |
...
- **BUSY** : pilote M2 seul, vitesse forcée lente (P1) ; ordre émis seulement sous `MotionRequestActive` ET sens ET **permis adapté au sens** ; sens inverse (recul) honoré sous le permis opposé — **et borné par la frontière physique de la benne** (`:559-588`) : seule la direction de recul est inhibée, `CloseReq`/`OpenReq`, `Lifecycle` et `BucketState` sont intacts, donc la reprise se fait au joystick sans réappui IHM. La limite est **directionnelle** : le sens allant vers la cible n'est jamais inhibé (le maintien AX10B, qui force déjà `M2_ReqDescend := FALSE`, est donc inaffecté)
- **DONE** : attend relâchement demande pour repasser READY
- **État publié** (`IsOpen`/`IsClosed`/`IsIntermediate`/`TooOpen`/`TooClosed`) : **classification continue** sur `Delta = M2 - M1` à chaque scan dès que M1 ET M2 sont référencés (`:413-445`) — plus d'état latché depuis T247 (commit `509e0eb1`)
...
| **5** | ✅ **CLOS (T346, 2026-09-20)** | **Perte de la borne de recul** : les deux branches d'arrêt à `M2StartPosM` ont été retirées le 2026-09-05 (commit `2a307b5b`, « suppression des coupures parasites ») ; `LeftStartSinceArm` reste **affecté** (`FB_Bucket.st:561`) mais n'est **plus jamais lu** (code mort) | ✅ **CORRIGÉ (T346)** : borne reconstruite sur la **frontière PHYSIQUE** de la benne (même frontière que `TooOpen`/`TooClosed`, `MinAllowedOffset` et la cause 1 — **aucune constante nouvelle**), confirmation `CST_BandStableTime`, **directionnelle**, sans toucher `Lifecycle` ni `BucketState` (reprise au joystick sans réappui IHM). Les **6 variables mortes sont purgées**. Preuves : <nobr><code>TC-P10-029.1</code></nobr> (était ROUGE) → VERT, + <nobr><code>TC-P10-T346-N1</code></nobr> et <nobr><code>TC-P10-T346-N2</code></nobr> **prouvées mutation-sensibles** ; CI FB_Bucket 38/38 ; garde-fou <nobr><code>G514</code></nobr> |
| **9** | ✅ **CORRIGÉ (T346, 2026-09-20)** | 🔴 **Défaite de détection de sécurité** : `M1RefPosM` est la référence de la **détection de glissement M1** (§1 cause 3) et était capturée sur le **même** front que la borne de recul. Tout relâchement/repompage du joystick en manœuvre la **rebaselinait**, découpant la dérive en tranches sous la tolérance : une dérive cumulée de 1,6 m (`M1SlipToleranceM` = 1,0 m) restait **invisible** | ✅ **CORRIGÉ** par la même clause `NOT Lifecycle.Busy` (le front d'armement reste préservé : `Lifecycle.Busy` est posé plus bas dans le même scan). Preuve : <nobr><code>TC-P10-T346-N2</code></nobr> ROUGE sans la clause / VERT avec |
| **10** | 🟠 **P2 — REMONTÉ (hors T346, non corrigé)** | **Deux témoins de sécurité morts, de la même famille que l'alerte 9** : `ContactorIncoherentError` (`FB_Brake.st:48`, « défaut mémorisé incohérence contacteur ») et `IhmEdgeSeen` (`FB_Diag_IhmHeartbeat.st:30`, « verrou front vu, interdit RETAIN au boot ») sont **écrits et jamais lus**. Deux autres cas du même type : `StepDelayTarget` (`FB_WinchOutputInterlock.st:106`, écrit 9 fois) et `PresetCompleted` (`FB_Encoder_Abs.st:60`) | ⛔ **Signalé, non corrigé** (hors périmètre T346 — à cadrer séparément). Détectables par <nobr><code>G514</code></nobr>, dont la portée est pour l'instant limitée aux fichiers du lot T346 |
