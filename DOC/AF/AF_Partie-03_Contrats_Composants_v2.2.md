# Analyse Fonctionnelle — Partie 3 : Contrats Composants (v2.2)
...
- **Type de composant** : Fondations méta — pas de FB unique porteur, mais §4.1 documente en
  détail le socle transverse `FB_FbStatus` (contrat `light`, réutilisé par tout FB `standard`).
...
|---|---|---|---|---|
| <nobr><code>TC-P03-001</code></nobr> | Priorité `Enable > SafeStop > StartStop` | `SafeStop` impose la rampe rapide même si `StartStop=TRUE` | `💻 AUTO` | <small>§4</small> |
...
| <nobr><code>TC-P03-008</code></nobr> | Cumul de plusieurs Fault latchés | 2 bits distincts apparus à des instants différents s'accumulent dans `ErrorId`, Reset les acquitte ensemble | `💻 AUTO` | <small>§4.1</small> |
| <nobr><code>TC-P03-009</code></nobr> | Sélection du texte IHM si Fault+Warning actifs ensemble | Texte IHM = première cause active (index le plus bas) par parcours de la liste `Causes` ; le warning reste dans `WarningIdTxt`, jamais dans `ErrorId` | `💻 AUTO` | <small>§4.1</small> |
| <nobr><code>TC-P03-010</code></nobr> | Bornes de la liste de causes `Causes[0..15]` | `Causes[0]` et `Causes[15]` correctement gérés, pas d'off-by-one | `💻 AUTO` | <small>§4.1</small> |
| <nobr><code>TC-P03-011</code></nobr> | `Reset` sans historique de défaut | Aucun effet parasite, `ResetRequested` reste `FALSE` | `💻 AUTO` | <small>§4.1</small> |
| <nobr><code>TC-P03-012</code></nobr> | `Reset` maintenu (niveau haut) pendant plusieurs scans | ⚠️ Faille identifiée (T148) : si la cause disparaît pendant que `Reset` reste haut sans nouveau front, l'acquittement se fait sans confirmation au moment réel — comportement actuel prouvé, pas validé comme cible | `💻 AUTO` | <small>§4.1</small> |
| <nobr><code>TC-P03-013</code></nobr> | Texte IHM pour un bit actif sans texte configuré | Chaîne vide, pas de plantage ni de texte résiduel d'un autre bit | `💻 AUTO` | <small>§4.1</small> |
...
(rempli via le socle `FB_FbStatus`, cf. §4.1).
...
### 4.1 Fiche `FB_FbStatus` — socle transverse de statut (forme cible)
...
Forme cible destinée à se généraliser aux autres FB `standard` (cf. §3).
...
| IN | `Reset` | `BOOL` | Front d'acquittement, jamais conditionné par un état externe (§4) |
