# 🏗️ Diagramme architecture cible — blocs & flux (d'après les specs AF, v1.1)
...
```mermaid
flowchart LR
    A["📥 01 PRG_02_Acquisition<br/>HwReal · HwSim · HwIn<br/>TOR · PDO · codeurs · joystick<br/>diag devices · état AU"]
...
## 2. Zoom `PRG_02_Acquisition` — frontière interne (AF06 §1/§2) — vue générale
...
flowchart LR
    ...
    subgraph A2["📥 PRG_02_Acquisition"]
...
> codeurs, décodeur M3, diag, AU) passe par **`HwIn`**.
...
| 3 | Le filtrage retenu est appliqué dans la frontière acquisition ; le sim est déjà normalisé | AF06 §2ter |
| 4 | `HwIn` est l'image unique consommée par les procédés | AF06 §2ter |
...
**Producteur** : `PRG_02_Acquisition.instJoystick`. **Sorties** : `ST_Joystick_AxisCmd` (X/Y)
**et** `DeadmanArmed` (BOOL séparé — **pas** dans le DUT).

```mermaid
flowchart LR
    subgraph ACQ["📥 PRG_02_Acquisition"]
...
**Producteur** : `PRG_02_Acquisition` (`instDiagCanOpen`, `instDiagEthercat`, `instIhmHeartbeat`).
...
|---|---|---|---|
| TOR réelles qualifiées | `HwIn` | 1 BOOL par TOR (`M1/M2/M3_*_DI`, `Machine_*`) | AF06 §2ter |
...
| Legacy | Cible | Lot |
|---|---|---|
| `PRG_00_Inputs` / `PRG_INPUTS_LD` | `couche historique PRG_01 (retraité)` | M1 |
| `PRG_ACQUISITION_CFC` + `PRG_02_Encoders` + `PRG_01_Diagnostics` + `PRG_AUXILIARY_CFC` | `PRG_02_Acquisition` | M1 |
| `instPositionDecoder` (acquisition) | `PRG_02_Acquisition` — sorties lues par `PRG_05` | M1 |
