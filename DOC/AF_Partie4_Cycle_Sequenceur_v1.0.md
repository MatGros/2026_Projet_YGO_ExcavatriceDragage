# 📋 Analyse Fonctionnelle — Partie 4 : Cycle & Séquenceur
...
>
> 🔗 Dépend de : Partie 2 v2.4 (architecture), Partie 3 (v1.1) (contrat FB), Partie 5 (modes).
...
`E_CycleStep`). `FB_Cycle` émet `Enable` / Sens / SpeedRef vers les treuils et la
translation selon l'étape active. Le passage à une étape **sans mouvement** retire l'`Enable`
→ arrêt naturel sur rampe (synchro ou non selon l'étape).
...
ENUM
  ...
  ERROR_HOLD           := 12;  (* Arrêt sûr figé sur défaut (CoupeEnable) — sortie par reset + maintenance *)
END_ENUM
END_TYPE
```

> 📌 `ERROR_HOLD` n'est pas une étape « normale » : c'est l'état de repli atteint dès que
> `FB_Safety.CoupeEnable` est actif. La reprise exige cause disparue + reset (front) + nouvel
> ordre explicite (jamais de redémarrage automatique — voir Partie 3 §Reset).
...
[à tout instant] CoupeEnable ──► ERROR_HOLD
```
...
> ⚠️ **TBD (To Be Defined)** : le détail fin de la séquence INIT reste à arrêter.
...
> `FB_WinchSync` est **actif tout le temps** (au minimum pour informer). En maintenance N2,
> son contrôle peut être **désactivé volontairement** (codeur mort, etc.) — voir Partie 5.
...
```
1. Commande automate moteur (contacteur sens + palier)
...
```
1. Couper la commande treuil (Enable retiré)
...
Exemple : opérateur a choisi "position 2" à l'IHM, puis avance au joystick.
  ...
  3. Arrêt EXACT sur le CAPTEUR de position (dérive négligeable),
       même si le joystick reste actionné.
...
(décalage de position relatif à M1).
...
- En cas de **perte** (codeur, bus, joystick), l'arrêt se fait sur une **rampe plus raide**
  mais **non destructive** (on garde les relais actifs le temps d'un ralentissement maîtrisé),
  puis collage des freins (voir §4) — voir aussi Partie 5 §Pertes/Défauts.

---

## 📚 Documents liés
- **Partie 2 v2.4** — Architecture (orchestration, FB_WinchSync, IO).
- **Partie 3 (v1.1)** — Contrat FB (E_State, ErrorId, reset).
- **Partie 5** — Modes & maintenance (overrides N2, limite légale, pertes/défauts).
