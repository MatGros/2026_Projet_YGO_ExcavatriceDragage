...
| T102 | 🔴 **Migration vers l'architecture 7 POU par procédé (M0→M8).** Cible : `PRG_01_Inputs_LD` · `PRG_02_Acquisition_CFC` (absorbe codéurs, diagnostics, auxiliaires, état AU) · `PRG_03_Modes_Cycle_CFC` · `PRG_04_Treuils_Benne_CFC` (safety M1/M2 intégrée) · `PRG_05_Translation_CFC` (safety M3 intégrée) · `PRG_06_Outputs_LD` (agrège `PowerCutOff`) · `PRG_07_Supervision_CFC` (absorbe le troubleshooting, lecture seule stricte). ⛔ **M7 (renumérotation) verrouillé** tant qu'un cycle inter-programme subsiste ; **M8 (CFC natif)** verrouillé tant que M7 n'est pas importé dans CODESYS. Ordre, criticités, grille d'évaluation et contrats de lot : `AUDITS/Architecture/PLAN_EXECUTION_MIGRATION_7POU.md` | Projet / Sécurité | AF02 §2/§4, T103 |
