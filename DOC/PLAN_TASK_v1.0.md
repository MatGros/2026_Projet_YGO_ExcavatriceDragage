...
| T89 | 🟡 **Offset fermeture benne à reconfirmer** (MES-010) : mesure terrain ≈ 15 m, appliquée le 2026-07-27 dans `_BucketCfgPersist.OffsetCloseM` (10.0 → 15.0). Le registre note la mesure comme « non encore analysée » → **valider au premier essai benne en charge**, ajuster si l'ouverture/fermeture ne se termine pas à la bonne cote | Terrain | `REGISTRE` MES-010, `GVL_PERSISTENT` |
| T90 | 🟡 **Hauteurs treuils à contrôler sur site** (MES-009) : capteur haut mesuré à 8,0 m, arrêt réel ≈ 7,5 m. Appliqué le 2026-07-27 : `CfgTopSensorPos_M := 8.0` (position inscrite par le homing au déclenchement du capteur) et `CfgCableLimitAscent_M := 7.5` (limite logicielle sous le capteur physique). ⚠️ Une erreur sur `CfgTopSensorPos_M` **décale toutes les positions machine** → vérifier au premier homing que `Position_M` correspond à la cote réelle | Terrain / Sécurité | `REGISTRE` MES-009, `GVL_PERSISTENT` |
| T91 | 🟡 **Réglage fin du délai de fermeture frein** (MES-006) : constat terrain de « glissement » de la charge au retour joystick au neutre en petite vitesse. Décision actée : **ne pas toucher aux rampes** (sécurité mécanique) ; le levier est le temps de retombée du frein. ⚠️ Dépend directement de **T87** — le paramètre `DelayMotorDecel` censé porter ce réglage **n'a aujourd'hui aucun effet**. Ajustement en présence du dragueur, essais en charge | Terrain | `REGISTRE` MES-006, `AUDIT_Revue_Technique` §6 |
