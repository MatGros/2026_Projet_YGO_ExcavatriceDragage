...
| 7 | Translation M3 — sécurité, sortie moteur, homme-mort | T104 + T105 + T106 + T107 | Audit 2026-08-05 (session M3). 4 contrats agent prêts, ordre au choix utilisateur : `TASK_CONTEXT_LOT0_DIAG_IHM_HEARTBEAT.yaml` (prérequis M4), `TASK_CONTEXT_M4_TRANSLATION_SAFETY.yaml`, `TASK_CONTEXT_LOT2_TRANSLATION_M3_AC600_OUTPUT.yaml`, `TASK_CONTEXT_LOT3_TRANSLATION_M3_DEADMAN.yaml` | ⬜ Documenté, aucun code écrit — attend décision d'ordre | — | ⏳ Attend priorisation utilisateur |
...
| T104 | 🔴 **`FB_Safety_Translation` jamais instancié** (audit M3 2026-08-05) : les 8 mécanismes (perte comm opérateur, perte comm EtherCAT, rotation phase, thermique frein, Méca A, Méca B, butées extrêmes, incohérence capteurs) sont écrits et corrects mais **aucun `instSafetyTranslationM3` n'existe dans `CODE/`**. `TranslationFinalInterlockRequest.SafeStop` (PRG_05) ne reçoit que `InputModuleFault` — 1 cause sur 8 — et aucun `PowerCutOff` M3 n'existe dans le programme. `PRG_06_Outputs_LD.st` le documente déjà en tête de fichier. Correspond au lot **M4** de `PLAN_EXECUTION_MIGRATION_7POU.md` (C4, rebuild, double revue). Contrat prêt : `DOC/CHECKLISTS/TASK_CONTEXT/TASK_CONTEXT_M4_TRANSLATION_SAFETY.yaml` (réécrit 2026-08-05, l'ancien référençait des fichiers CFC disparus) | Projet / Sécurité | `CODE/TRANSLATION/FB_Safety_Translation.st`, `CODE/MAIN/PRG_05_Translation.st`, `CODE/MAIN/PRG_06_Outputs_LD.st:16-22` |
| T105 | 🔴 **Commande moteur AC600 M3 jamais écrite en sortie physique** (audit M3 2026-08-05) : `FB_TranslationOutputInterlock_LD` calcule `DriveControlWord`/`DriveFreqRefHz` (VAR_OUTPUT) mais rien ne les écrit vers un PDO EtherCAT — seul `BrakeCmd` est câblé (`M3_BrakeRelease_RQ`). M3 ne peut physiquement pas bouger même si `SafetyStructureNotValidated` est un jour levé. Contrat prêt : `DOC/CHECKLISTS/TASK_CONTEXT/TASK_CONTEXT_LOT2_TRANSLATION_M3_AC600_OUTPUT.yaml` — ⚠️ dépendance croisée avec T104 : câbler ceci sans T104 fait bouger le moteur sans les 8 protections | Projet / Sécurité | `CODE/TRANSLATION/FB_TranslationOutputInterlock_LD.st`, `CODE/MAIN/PRG_06_Outputs_LD.st` |
| T106 | 🟠 **`DeadmanArmed` (homme-mort joystick) non lu par `PRG_05_Translation`** (audit M3 2026-08-05) : `M3_StartStop_Active` n'exige `DeadmanArmed` ni en SEMI_AUTO ni en MAINT_N1/N2 boutons IHM, contrairement à `AF_Partie-11 §4.2` et à l'écart déjà noté au point "2.A" de ce document. Écart probablement identique côté M1/M2 (Winch), à vérifier séparément. Contrat prêt : `DOC/CHECKLISTS/TASK_CONTEXT/TASK_CONTEXT_LOT3_TRANSLATION_M3_DEADMAN.yaml` | Projet / Sécurité | `CODE/MAIN/PRG_05_Translation.st`, `CODE/JOYSTICK/FB_Joystick.st`, point "2.A" (ligne 97 de ce document) |
| T107 | 🟠 **`FB_Diag_IhmHeartbeat` jamais instancié** (audit M3 2026-08-05, trouvé en creusant T104) : `HeartbeatIhmOk` est lu par `FB_Cycle`, `FB_Safety_Winch` et `FB_Safety_Translation` mais son seul producteur (`FB_Diag_IhmHeartbeat`) n'est appelé nulle part — `GVL_IHM.Commun.HeartbeatIhmOk` n'a aucune assignation dans `CODE/MAIN/`. Prérequis de T104 : sans ce lot, câbler `HeartbeatIhmOk` dans FB_Safety_Translation reviendrait à brancher une entrée sur du vide. Contrat prêt : `DOC/CHECKLISTS/TASK_CONTEXT/TASK_CONTEXT_LOT0_DIAG_IHM_HEARTBEAT.yaml` | Projet / Sécurité | `CODE/DIAG/FB_Diag_IhmHeartbeat.st`, `CODE/SUPERVISION/_TYPES/ST_CommunHMI.st:29` |
