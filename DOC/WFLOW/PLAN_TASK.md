...
| **T123-VAL** | Validation & Bundle | `run_all_gates.py` + Régénération `CODE_Bundle_v2.xml` | ✅ | AGY-01 | ✅ Validé (15/15 Gates PASS, Palier C, G200 0 erreur) |
...
| T93 | Remplacer rampe %/s par temporisations de maintien entre paliers contacteurs | Projet / Maintenance | ✅ | DSH-02 | `FB_SpeedStep`, `FB_Winch` (Paliers contacteurs et hystérésis intégrés) |
...
| T125 | Revue conception modes dragage (DiveSearch / ExtractionSequence / DumpAtTremie) — standard projet + fonctionnel après essais | Projet / Sécurité / IHM | ⬜ | — | Session 2026-08-18 : logique DumpAtTremie **inline dans `PRG_04`** (violation « 1 FB = 1 responsabilité ») ; **verrou translation à la trémie absent** ; **latch « une fois descendu → translation interdite » (P1/Maintenance) absent** ; DiveSearch/ExtractionSequence à revoir (standard + fonctionnel). Traitement en 2e temps. <br>**Adossé (2026-08-21, décision utilisateur)** : regrouper les **types des séquences/cycles** (ST_DredgingAssistCfg/State/Cmd/HMI, et tout type `ST_Cycle*`) actuellement dans `J_SUPERVISION/_TYPES/5_ASSISTANCE_DRAGAGE` et `3_CYCLE_ET_MODES`, vers **`G_CYCLE`** (avec leurs FB DiveSearch/ExtractionSequence/Cycle déjà là). Cohérence : un cycle/séquence = son FB + ses types au même endroit. À faire **pendant** ce refactor (pas en tâche isolée). |
...
| T136 | **Écrire les 2 contrats d'interface FB** (`light` / `standard` + DUT `ST_FbStatus`) dans `CODE_QUALITY_STANDARDS.md` + `guard:` de vérification — **doc seul, zéro ligne `CODE/`** | Projet / Convention | ✅ | AGY-01 | **Implémenté & Validé 2026-08-19** : (1) Contrats light et standard + struct `ST_FbStatus` (6 membres) documentés dans `CODE_QUALITY_STANDARDS.md §2quinquies` ; (2) doublon 2bis résolu ; (3) déduplication Ladder dans `AF_Partie-03` ; (4) script de garde mécanique `G315_check_fb_interface.py` créé et validé + unittests OK. G315 type-aware + DUT ST_FbStatus créé. Validé (16 standard, 28 light). |
| T137 | **Migrer les 16 FB standard existants vers `ST_FbStatus`** — lot dédié, **séparé de T130/T136** | Projet / Convention | ✅ | DSH-02 | ✅ **Prérequis levé (2026-08-19)** : `G315_check_fb_interface.py` corrigé — il ne détectait que les membres **à plat** et ignorait `ST_FbStatus`, si bien qu'un FB migré retombait en « light » et que le script **sortait en succès sans alerter** (garde-fou dégradé en silence dès le 1ᵉʳ FB migré). Corrigé + 4 tests de non-régression. **Bonus** : le rapport distingue désormais *forme cible* / *forme héritée* → le guard sert d'**indicateur d'avancement T137** (migration finie quand la forme héritée tombe à 0 ; aujourd'hui **0 cible / 16 héritée**). **Arbitrage humain 2026-08-19 : `ST_FbStatus` est une CIBLE (option A)**, pas une forme alternative — d'où le maintien de cette tâche. Session 2026-08-19 — **144 lignes** mesurées (`PRG_04` 105 · `PRG_07` 20 · `PRG_05` 10 · `PRG_03` 5 · `PRG_02` 3 · `PRG_06_Outputs` 1 **hors périmètre** : `instSafetyEmergencyManagement.State` est un `ST_Safety_Emergency_State`, pas la phase `E_State` → **zéro risque import Ladder**). ✅ **Impact pupitre/SCADA = 0** (GVL_IHM, `GVL_Troubleshooting`, `FB_TroubleshootingView` lisent des structs), **MAIS `PRG_07` est édité** : il lit des FB **directement** → GVL_IHM (`instCycleSemiAuto.*`, `instDiag_CanOpen/instDiagEthercat.*`, `instHomingM1/M2.Busy`, `instWinchSync.Error`) — la double couche `FB → struct → GVL_IHM` n'est **pas universelle**. — seuls les **membres droits** changent, `GVL_IHM` et le pupitre sont intacts → **pas de transition en miroir**, migration franche. 🟢 100% rattrapé par le compilateur. ⚠️ **Ne pas mélanger avec T130/T135** : les deux tapent lourdement dans `PRG_04`, le `git diff` deviendrait irrelisible (impossible de distinguer un renommage mécanique d'un vrai changement de logique — rédhibitoire sur du code sécurité). Pilote proposé : **`FB_Translation`** (10 lignes) plutôt que `FB_Winch` (105). <br>**✅ CLÔTURÉ (2026-08-21)** : migration des **16 FB** terminée — `G315` forme cible=**16** / héritée=**0** · G200 PASS · gates 15/15 · revue indépendante PASS (mécanique, bitmasks identiques). Commité `332eb01`. Pilote `FB_Translation` + 15 FB migrés. ⚠️ `FB_WinchLoadEstimator`/`FB_Encoder_SpeedMonitor` migrés mais non instanciés (provisionnés). Bénéfice réel : *namespacing* `Status.State` (lecture plus explicite) ; ⚠️ **ne supprime PAS** le contournement `FBState` (champ de struct sans conflit compil avec `FB.State`) — bénéfice surévalué dans la version précédente. 🔗 **Intègre T143 (2026-08-20)** : lors de la migration de `FB_Safety_Winch`/`FB_Safety_Translation` vers `ST_FbStatus`, **conserver l'`ErrorId`** (ne pas le reset sur `Enable=FALSE` / mode DISABLE) pour rendre la cause Meca visible — le correctif minimal `Enable := TRUE` est documenté dans T143 |
...
| T142 | **Encapsulation des échanges inter-PRG par bus DUT typés** (Phasage étanche : P1=PRG_02, P2=PRG_04, P3=PRG_05, P4=PRG_06) | Projet / Architecture | ✅ | DSH-02 | **✅ Clôturé & validé (2026-08-20)** : les 4 PRG exposent `Data : ST_xxxInterPrg` (PRG_02/04/05/06), sorties plates retirées, consommateurs remappés, champ mort `SimulationBypassActive` nettoyé. Bundle fresh · G200 PASS · **Gates Palier C 15/15 PASS**. Validé par l'utilisateur (clôture). |
| T143 | **Meca DriftGuard se déclenche au référencement sans mouvement** | Projet / Sécurité / Simulation | ⬜ | — | **Identification (2026-08-20)** : à l'appui du bouton de référencement M2 (benne ouverte/fermée) ou M1 (codeur), **sans mouvement**, MecaA se déclenche via `DriftGuardA.Violation` → PowerCutOff → contacteur coupé → mode DISABLE. Le bypass MecaA (IHM) permet le référencement. <br>**Cause probable** : `InReferencingMode := instHomingM2.Busy` (PRG_04 l.736) mal synchronisé — DriftGuardA armé (`NOT InReferencingMode`, FB_Safety_Winch l.283) au moment de l'appui → détecte une dérive transitoire. **L'ErrorId n'est pas conservé** (défaut de conception) → cause invisible au snapshot. <br>**Solution proposée** : ① vérifier/corriger la synchro `InReferencingMode` pendant le référencement ; ② conserver l'`ErrorId` (latch cause) jusqu'au Reset pour rendre la cause visible. <br>🔗 **Intégré à T137 (2026-08-20)** : le correctif définitif (conservation `ErrorId` + interface `ST_FbStatus`) sera porté par la migration `FB_Safety_Winch`/`FB_Safety_Translation`→`ST_FbStatus`. Correctif minimal proposé : `Enable := TRUE` (surveillance permanente) sur les 3 FB safety. <br>**Session 2026-08-21 — cause racine CONFIRMÉE** : le « Meca à l'arrêt » (M1/M2) vient d'une **course temporelle** entre l'**armement instantané** (contacts/frein échos de commande sim) et la **fenêtre de mesure vitesse 50 ms** (`FB_Encoder_SpeedMeasure`) → `MeasuredSpeedMps > 0,02` ~50 ms après l'arrêt → faux Meca bit7. Même famille pour `NoMovement` (bit15, vitesse < 0,02 pendant commande). **Correctifs appliqués & commités** : ① `TON arrêt confirmé 100 ms` avant d'armer la détection de vitesse Meca (FB_Safety_Winch) ; ② `Reset effectif en DISABLE` (efface la cause même en mode neutre → réarmement) ; ③ `SimulationBypassActive` étendu aux bypass **Safety+Process** M1/M2 (banc). ⏳ **Référencement toujours à investiguer** (synchro `InReferencingMode`). |
| T144 | **Assainir l'interface de `PRG_06_Outputs`** (conformité esprit bus/encapsulation T142 + sorties M3 mortes) | Projet / Architecture | ⬜ | — | **Signalé 2026-08-20** (revue sim M3) : PRG_06 ne respecte pas l'esprit bus (T142). Faits vérifiés : ① `M3_DriveControlWord`/`M3_DriveFreqRefWord` (VAR_OUTPUT l.43-44) **jamais assignés** (morts, toujours 0) ; ② `M3_CommandWord`/`M3_SetpointFrequencyHz` (assignés l.215-216) **non déclarés dans les `.st`** (globals uniquement dans `Device.export`) → source non auto-consistant ; ③ la **commande drive M3 n'est PAS dans le bus `Data`** (`ST_OutputsInterPrg` ne porte que `TranslationBrakeCmd`) ; ④ **diag plats en VAR_OUTPUT** (`M3BlockedBySafetyInfo`, `EmergencyState`, `EmergencyDiag`…) au lieu d'un struct/bus diag propre. <br>**Objectif** : aligner PRG_06 sur l'encapsulation (bus `Data` + struct diag), assigner/supprimer les sorties mortes, déclarer les globals dans les `.st`, exposer la commande drive M3 via `Data` (source sim fiable aussi). **Lot séparé** de la sim (qui utilise déjà `PRG_05.Data.TranslationFinalInterlockRequest`). |
| T145 | **Refonte du suivi des tâches : Source unique YAML (`TASKS.yaml`) + Vue tableur CSV en lecture seule** | Outillage / Workflow | ⬜ | — | **Étude 2026-08-21** : Remplacer le suivi Markdown par un fichier source structuré unique `DOC/WFLOW/TASKS.yaml` (élimine le non-déterminisme des LLM sur les tableaux `.md` et les conflits d'édition) couplé à une génération automatique d'une vue de consultation `DOC/WFLOW/TASKS_VIEW.csv` (filtrable et triable dans VS Code via l'extension `Edit CSV` ou Excel). Garde-fou associé : Gate CI `G350` de validation d'intégrité. <br>**Décision** : Mise en réserve, à introduire ultérieurement si le volume de pilotage le nécessite. |
| T146 | **Référencement / homing des codeurs : état incohérent & suspect — mise à plat, challenge & simplification** | Projet / Sécurité / Convention | 🔍 | DSH-02 | **Signalé 2026-08-21** (revue interlock hauteur M3 `M3_HeightInterlockOk`) : les concepts « codeur incohérent » (`FB_Encoder_Safety.EncoderIncoherent` = **hors bornes** OU **HomingSuspect**) et « codeur référencé » (`FB_Encoder_Homing.Homed` = `Calib.Homed` **AND NOT** `HomingSuspect`) se recouvrent mais ne sont **pas identiques**. Cas trouvé : un codeur **jamais homé mais dans les bornes** → `EncoderIncoherent = FALSE` → sa position (non fiable) est **crue** par l'interlock hauteur M3 (défaut fail-safe). <br>**Objectif** : ① mettre à plat et **challenger** la machine à états référencement (`FB_Encoder_Homing` / `FB_Encoder_Safety`) ; ② clarifier les définitions **incohérent / suspect / référencé** et leurs conditions d'armement/retombée ; ③ simplifier la compréhension via de **petites fonctions** dédiées (ex. helper « position fiable pour l'interlock » exposant `Homed`/`Incoherent`/`CablePosMSafe`). **Phase étude / plan — zéro ligne `CODE/`** tant que non validé. Dépendance : à trancher avec le correctif d'écriture `M3_HeightInterlockOk` (Homed vs Incoherent). |
| T147 | **`FB_FbStatus` : fault latché perdu (silencieusement) sur un cycle `Enable=FALSE→TRUE` sans `Reset`** | Projet / Sécurité / Convention | ⬜ | — | **Audit indépendant 2026-08-22** (test_auto_ci + revue sécurité machine, sur `CODE/A_COMMUN/FB_FbStatus.st`, en cours de développement par un autre agent — pas de code touché ici). <br>**Constat** : le gate `IF NOT Enable THEN ... ErrorIdLatched := 0; ... END_IF;` (§1, ~ligne 55) remet la mémoire de défaut latché à zéro dès que `Enable` repasse à `FALSE`, **sans passer par un front `Reset` explicite**. Ça viole l'esprit "Reset = front conscient, jamais de réarmement/acquittement accidentel" (AGENTS.md). Scénario concret : un fault latché sur un FB standard (ex. futur treuil) ; l'opérateur change de mode ou un re-init logiciel fait retomber `Enable` un cycle puis remonter ; le fault disparaît de l'IHM sans qu'aucun `Reset` n'ait été appuyé → perte de traçabilité d'un défaut réel, l'opérateur croit la machine saine. <br>**Décision utilisateur (2026-08-22)** : la mémoire du dernier défaut latché **ne doit pas** être remise à zéro par `Enable=FALSE` — seul un front `Reset` explicite doit acquitter. <br>**Correctif proposé** : retirer `ErrorIdLatched := 0;` de la branche `IF NOT Enable THEN` dans `FB_FbStatus.st` §1 — le reste du gate neutre (façade `Status.Error/ErrorId` à 0 tant que `Enable=FALSE`) peut rester tel quel, seule la mémoire interne doit survivre au cycle désactivé. <br>**Doc à mettre à jour en parallèle** (`DOC/STDS/CODE_QUALITY_STANDARDS.md` §3bis) : documenter explicitement ce comportement (Enable=FALSE ne doit pas acquitter, seul Reset le fait) — actuellement non écrit nulle part (ni code ni standard) : trou d'audit distinct de l'ambiguïté elle-même. <br>**Autres trous identifiés dans le même audit (non bloquants, à trier séparément si besoin)** : cumul de 2 bits latchés jamais testé, priorité d'affichage texte fault vs warning simultanés non tranchée, standard `CODE_QUALITY_STANDARDS.md` décrit une "rotation d'affichage 1s" que le code n'implémente pas (§3bis, à corriger ou implémenter), `Busy`/`Done` "hors socle" non documenté dans le standard, `WarningIdCause` cité dans le standard mais absent de l'interface réelle (coquille). |
| T148 | **`FB_FbStatus` : Reset simplement MAINTENU (sans nouveau front) + disparition de la cause pendant ce maintien = acquittement silencieux** | Projet / Sécurité | ⬜ | — | **Découvert en écrivant les tests étendus `test_fb_fbstatus.st` (2026-08-22)** — même famille que T147, faille distincte (pas de code touché, `TOOLS/` uniquement). <br>**Constat** : `ResetEdge(CLK:=Reset)` ne (re)vide `ErrorIdLatched` qu'au **front montant** de `Reset` — mais rien n'empêche ce vidage de se produire alors que la cause brute (`FaultCause`) est **encore active** à cet instant précis (le test `TC` "Reset ne doit pas acquitter tant que la cause persiste" ne le voit pas car il ne teste qu'1 seul cycle : `Error` reste bien `TRUE` cette scan-là grâce à l'union sur la cause brute, pas grâce au latch). Séquence qui casse la garantie : Reset **maintenu à TRUE** plusieurs scans pendant que la cause est active (le latch est vidé au 1er front et jamais réarmé, faute de nouveau front `FaultEdge` puisque la cause reste constante) → si la cause **disparaît** À CE MOMENT-LÀ (Reset toujours haut, donc **aucun nouveau front Reset**), `Error` retombe silencieusement à `FALSE` — sans confirmation opérateur au moment réel où le défaut se résout. <br>**Scénario concret** : opérateur bloque/maintient le bouton Reset (bouton collé, séquence auto qui garde Reset actif) pendant qu'un défaut réel est encore présent ; le défaut se résout tout seul (cause disparaît) pendant ce maintien → l'IHM efface le défaut sans qu'aucun nouveau geste conscient n'ait eu lieu à l'instant de la résolution. Prouvé par test (`TOOLS/TEST_AUTO_CI/RESULTS/A_COMMUN/tests/test_fb_fbstatus.st`, test *"Reset MAINTENU plusieurs scans..."*), comportement actuel documenté dans le test, pas un choix de spec validé. <br>**Piste de correctif à trancher** (pas encore décidée) : ne vider `ErrorIdLatched` sur le front `Reset` que si `FaultCause = 0` à cet instant (sinon ignorer le front tant que la cause est active), pour que seul un **nouveau** front `Reset` survenant **après** disparition de la cause puisse acquitter — cohérent avec le test déjà existant "Reset ne doit pas acquitter tant que la cause persiste" mais étendu à Reset maintenu. À trancher avec la même décision de spec que T147. |
| T149 | **État des lieux & reliquats Joystick (première étape d'intention utilisateur)** — point de référence du domaine joystick, prélude à l'intention de mouvement (T130/T135) | Projet / Sécurité / IHM | 🔍 | DSH-02 | **Session 2026-08-22** — assainissement + dynamique du joystick simulé. **Accompli (commit)** : ① `FB_Joystick` migré contrat `standard` → `Status : ST_FbStatus` (socle `FB_FbStatus`), retrait `PowerContactorEngaged` (device, pas d'organe), retrait `FB_Filter_PT1` (joystick stable → `FB_Filter` A_COMMUN), `RawOutOfRangeMargin` en paramètre `[CFG]`, surveillance capteur continue + `Enable:=FALSE` sur défaut capteur, perte bus → `ErrorId` bit2 auto-effacé. ② Simulation : `FB_Sim_Joystick` **dynamique** (montée progressive 1-2s + pseudo-aléatoire, retombée 200ms, bagotement `SimJoystickBagotement_Active`, overshoot `SimJoystickOvershoot_Active`) ; correctif gating `FB_SimBench` (retombée/bagotement masqués après relâchement) validé par test CI ad hoc (4/4) + trace CODESYS. ③ Outillage : dossier `_TROUBLESHOOTING/` (test CI ad hoc hors registry) + méthode skill troubleshooting (§4ter + orientation). <br>**Reliquats joystick** : ① décision **désarmer homme-mort sur défaut capteur** (P1 sécurité, AF §5bis non tranché — attente utilisateur) ; ② architecture `FB_FbStatus`/`ST_FbStatus` enrichi (Warning/textes) vs 6 membres historiques (impact 16 FB, T137) ; ③ fiabiliser gate homme-mort au niveau des FB aval (T130/T135) ; ④ IHM `ErrorIdTxt`/`WarningIdTxt` si enrichissement adopté ; ⑤ **T130** standardiser décodage d'intention → `{Direction, Palier}` (étape d'intention de mouvement, après l'intention de geste validée ici). <br>⚠️ **Rappel sécurité** : bypass commun de sécurité doivent rester OFF en exploitation (RETAIN). |
