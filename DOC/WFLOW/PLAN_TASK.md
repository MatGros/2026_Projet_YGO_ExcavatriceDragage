...
| **`PowerCutOff` agrégé en sortie** *(2026-08, actée)* | Chaque procédé publie **sa demande** ; `PRG_06_Outputs_LD`, seul au plus près des sorties, agrège et coupe. Aucun POU « safety machine globale ». La chaîne AU matérielle reste indépendante et prioritaire (Partie 01) |
...
| T70 | ✅ **Tranché (décision utilisateur 2026-08-03)** : retour en MAINT_N1 au boot **voulu** — valeur restrictive par sécurité, aucune correction. | Projet / Client | AF_Partie-11 §4 |
...
| T99 | 🟡 **`GVL_IHM_AU` détectée orpheline (LOT_A_SUPPRESSION_CODE_MORT, 2026-08-01).** Même origine que `FB_Sim_AU_ChainFeedback`/`GVL_Simulation_AU` (banc de test AU jamais raccordé, cf. `PRG_AU_TestBench.st` archivé) : aucun champ `Cmd`/`State` jamais lu/écrit hors de son propre fichier. **Hors périmètre explicite du lot** (contrat de tâche restreint à 3 objets nommément désignés) — tracée en exemption `KNOWN_ORPHANS_PENDING_DECISION` dans `check_linkage.py` (WARN visible, pas d'erreur bloquante). **Décision à prendre par l'utilisateur** : supprimer avec `ST_Safety_Emergency_HmiCmd`/`HmiState` (même sort que les 3 objets déjà retirés) ou conserver si un usage futur est prévu. ✅ **Tranché 2026-08-03 (utilisateur) : SUPPRIMER** — à exécuter dans un lot de suppression dédié (code mort confirmé, aucun consommateur) | Projet | `CODE/SUPERVISION/GVL_IHM_AU.st`, `TOOLS/AGENT_WORKFLOW/scripts/check_linkage.py` (L13) |
| T97 | 🟠 **Refonte acquisition — décision documentaire 2026-08-04.** `PRG_02_Acquisition` devient le producteur unique de `HwReal`/`HwSim`/`HwIn` et des diagnostics modules. `PRG_01_Inputs_LD`, `FB_Input` et `ST_InputsQualified` passent en retrait contrôlé. Prérequis avant code : inventaire des consommateurs, preuve du filtrage matériel ou déplacement du filtre dans `PRG_02`, remappage `PRG_06`/`PRG_07`, validation liaison et essais AU/freins/thermiques/simulation | Projet / Sécurité | `DOC/AF/AF_Partie-06_Acquisition_Qualification_IO_v2.3.md`, contrat LOT_RATIONALISATION_ACQUISITION_FB_INPUT |
| T98 | 🟠 **`BrakeThermalFault`/`PhaseRotationFault` (`ST_CommunHMI`) déclarés mais jamais câblés vers l'IHM** — même bug de câblage que `HydraulicThermalFault` (corrigé LOT_B 2026-08-01, `PRG_SUPERVISION_CFC.st`). Les deux champs existent dans `ST_CommunHMI` (diagnostic seul, aucune commande PLC) et leurs sources brutes sont déjà lues et consommées par la sécurité (`PRG_01_Inputs_LD.BrakeThermalFeedback` → `FB_Safety_Winch`/`FB_Safety_Translation`, `PRG_01_Inputs_LD.PhaseRotationOk` → mêmes FB), mais **aucune assignation** `GVL_IHM.Commun.BrakeThermalFault :=` / `GVL_IHM.Commun.PhaseRotationFault :=` n'existe dans `PRG_SUPERVISION_CFC.st` : l'opérateur ne voit jamais ces défauts sur l'IHM alors que la sécurité les traite déjà. **Hors scope explicite du LOT_B** (refusé par l'utilisateur) — documenté seulement, câblage à faire dans un lot dédié (même pattern qu'`HydraulicThermalFault` : `Champ := NOT PRG_01_Inputs_LD.<Source>` ou équivalent, polarité à revérifier par champ) | Projet | `CODE/SUPERVISION/_TYPES/ST_CommunHMI.st`, `CODE/MAIN/PRG_07_Supervision.st`, `CODE/TREUILS/FB_Safety_Winch.st`, `CODE/TRANSLATION/FB_Safety_Translation.st` |
| T101 | ✅ **Clos par la décision d'architecture par procédés (actée).** Les cycles prouvés (Acquisition↔Diagnostics, Acquisition↔Encoders, Supervision↔Diagnostics/Encoders, Safety↔Treuils/Translation) et les doubles producteurs joystick/codeurs ne sont plus traités par une scission Safety pré/post-mouvement : le découpage **par ensemble mécanique** les supprime par construction (chaque procédé porte sa safety). ⚠️ **Aucun renommage `PRG_XX` ni CFC XML sans lot dédié** — suivi en T102. Décision : `AUDITS/Architecture/RU_C4_ARCHITECTURE_PROCEDES.md`. Dossiers RU-4/C4 fondés sur la safety séparée : archivés dans `ARCHIVES/Doc/AUDITS/Architecture/` | Projet / Sécurité | AF02 §2/§4 |
| T102 | 🔴 **Migration architecture acquisition — lot dédié.** La cible documentaire retire la couche d'entrées `PRG_01_Inputs_LD` et laisse `PRG_02_Acquisition` en tête de la tâche, avant Modes, procédés, Outputs et Supervision. Aucun retrait CODESYS ne démarre avant validation humaine du dossier documentaire et du remappage ; `Device.export` reste interdit de modification | Projet / Sécurité | `DOC/AF/AF_Partie-02_Architecture_Programme_v3.1.md`, contrat LOT_RATIONALISATION_ACQUISITION_FB_INPUT |
| T103 | ✅ **TRANCHÉ (A-01, décision utilisateur 2026-08-03) — le homing lit le mode de marche.** Fait prouvé : `instHomingM1/M2` reçoivent `PRG_MODES_CFC.Auth.Mode`, `UnitaryMode` et `WinchSelected` (legacy `PRG_02_Encoders.st:72,74-76,122,124-126`, POU supprimé au lot M1) ; même lecture de `Auth.Mode` sur `instEncoderAbsM1/M2` (`:55,105`) et `instEncoderSafetyM1/M2` (`:155,168`). Or dans la cible l'acquisition est au rang **02** et les modes au rang **03** — le consommateur passerait avant son producteur. **Résolution retenue (option C, A-01)** : le homing migre en **lot M3 (Treuils/Benne, rang 04)** avec le mouvement qu'il recale — la dépendance vers Modes (rang 03) devient saine, et l'acquisition (rang 02) ne porte que la mesure. Plus aucun blocage M1. Preuve : `REGISTRE_ARBITRAGES_MIGRATION.md` §A-01, `PLAN_EXECUTION_MIGRATION_7POU.md` §M1 point dur 2 | Projet / Sécurité | AF09 §4bis, A-01, T102 (lot M1) |
| T100 | ✅ **Gate `check_linkage.py` L9 (mapping E/S) corrigé — 72 KO → 0 KO (2026-08-01, hors périmètre codex confirmé par grep des `TASK_CONTEXT_*.yaml`, aucun n'édite `check_linkage.py`/`linkage_gates_l8_l12.py`).** Cause racine confirmée : `load_io_mapping()` (`linkage_gates_l8_l12.py`) ne construit des clés que pour `PRG_OUTPUTS_LD.*`/`PRG_01_Inputs_LD.*` (seul point de contact réel avec le CSV), mais L9 comparait **tout** `VAR_OUTPUT` de **tout** PROGRAM contre cette table — faux positif garanti par construction pour ~64 des 72 erreurs (signaux internes calculés : `HydraulicFaultOk`, `Auth`, `FaultMachineReset_IHM`, `WinchM1FinalInterlockRequest`...). **Fix appliqué** : `L9Checker.check()` restreint aux deux POU physiques ; les 22 erreurs restantes (réellement dans `PRG_OUTPUTS_LD`/`PRG_01_Inputs_LD`) sont un vrai écart de nommage — le CSV utilise le libellé matériel brut (`M1_RelayFwd_Up_DQ`), le code le nom métier retraité (`M1RelayFwd`) — **aucune correspondance auto inventée** (règle projet), downgradées en WARN non bloquant à rapprocher manuellement. `check_linkage.py --report` = PASS, bundle régénéré, `run_all_gates.py` : GATE 2bis (LIAISON) PASS, PyTest 409 passed/8 skipped. **Reliquat** : les 22 WARN L9 (écart nommage CSV/code) restent à trancher par l'utilisateur — pas un bug outil, juste un rapprochement manuel non fait. ✅ **Tranché 2026-08-03 (utilisateur) : le CSV `Device_IO_20260729.csv` est la référence correcte.** L'écart vient du code (noms métier retraités) vs CSV (libellé matériel brut) — à rapprocher lors du lot de nommage/renommage E/S, sans correspondance auto inventée (règle projet) | Projet / Outillage | `TOOLS/AGENT_WORKFLOW/config/Device_IO_20260729.csv`, `TOOLS/AGENT_WORKFLOW/scripts/check_linkage.py` (classe `L9Checker`) |
...
| T96 | ✅ **Résolu (constaté lors de l'audit M3 2026-08-05)** — `M3_ThermalFeedback_DI` câblé mais jamais lu (audit nommage 2026-07-30) : disjoncteur thermique **moteur/variateur AC600**. **Preuve** : `ST_HwTranslation.M3_ThermalFeedback_DI` lu dans `PRG_02_Acquisition.st` (`HwReal`→`HwIn`), publié en diagnostic IHM par `PRG_05_Translation.st:164` (`TranslationStateHMI.DriveThermalOk`). Diagnostic seul, comme spécifié — aucune escalade `SafeStop`/`PowerCutOff`, conforme à la décision 2026-07-30 | Projet / Électricité | `CODE/SUPERVISION/_TYPES/ST_HwTranslation.st`, `CODE/MAIN/PRG_02_Acquisition.st`, `CODE/MAIN/PRG_05_Translation.st:164` |
| T104 | ✅ **Résolu (M4, 2026-08-05)** — `instSafetyTranslationM3 : FB_Safety_Translation` instancié dans `PRG_05_Translation.st:21,121`, appelé AVANT `instTranslationM3` (même lag 1 scan que le pattern legacy documenté AF11 §5). Les 8 mécanismes agrégés dans `M3_SafeStop_Aggregate` (`:156`), injectés dans le `SafeStop` du FB mouvement ET dans `TranslationFinalInterlockRequest.SafeStop`/`.PowerCutOff` (`:191`). `TranslationSafetyHMI.*` recopié depuis `instSafetyTranslationM3` (producteur unique, `:233-250`), plus aucun recalcul local | Projet / Sécurité | `CODE/TRANSLATION/FB_Safety_Translation.st`, `CODE/MAIN/PRG_05_Translation.st:21,121,156,191,233-250` |
| T105 | ✅ **Résolu (LOT3, 2026-08-05)** — `FB_TranslationOutputInterlock_LD` publie désormais `DriveFreqRefWord` (WORD, échelle ×100 confirmée terrain : `DriveFreqRefWord := REAL_TO_WORD(LIMIT(0.0, DriveFreqRefHz, 600.0) * 100.0)`, `FB_TranslationOutputInterlock_LD.st:163`) en plus de `DriveControlWord`. `PRG_06_Outputs_LD` capture les deux dans `M3_DriveControlWord`/`M3_DriveFreqRefWord` (oracle `gen_prg06_oracle.py`, pattern `<expression>` interne au bloc, identique à `TranslationBrakeCmd`). **Reliquat non-code, action terrain utilisateur** : mapping E/S CODESYS manuel restant (`M3_CommandWord` %QW6 ← `M3_DriveControlWord`, `M3_SetpointFrequencyHz` %QW7 ← `M3_DriveFreqRefWord`), documenté en note d'application dans `PRG_06_Outputs_LD.st` §2. ⚠️ Dépendance croisée avec T104 (résolu) déjà câblée : `SafetyStructureNotValidated` reste le garde-fou global, non touché par ce lot | Projet / Sécurité | `CODE/TRANSLATION/FB_TranslationOutputInterlock_LD.st`, `CODE/MAIN/PRG_06_Outputs_LD.st`, `TOOLS/ST_PLCOPENXML_GENERATOR/scripts/gen_prg06_oracle.py` |
| T106 | ✅ **Résolu (LOT3, 2026-08-05)** — `PRG_02_Acquisition` publie `JoystickDeadmanArmed := instJoystick.DeadmanArmed` (`:20,317`). `PRG_05_Translation.st:74` ajoute `AND PRG_02_Acquisition.JoystickDeadmanArmed` au calcul `M3_StartStop_Active` (branches MAINT_N1/N2). Branche SEMI_AUTO : commentaire explicatif seulement (`M3_StartStop_Active` déjà figé à FALSE, `FB_Cycle` non câblé — pas de fausse promesse sur du code mort). Écart M1/M2 (Winch) non traité, hors périmètre de ce lot | Projet / Sécurité | `CODE/MAIN/PRG_05_Translation.st:74`, `CODE/MAIN/PRG_02_Acquisition.st:20,317` |
| T107 | ✅ **Résolu (LOT0, 2026-08-05)** — `instDiagIhmHeartbeat : FB_Diag_IhmHeartbeat` instancié dans `PRG_07_Supervision.st:28,66`, publie `GVL_IHM.Commun.HeartbeatIhmOk`/`HeartbeatIhmTimeout`/`HeartbeatIhmElapsed`/`TglHeartbeatPlc` (`:183-186`). `T104` (safety M3, dépendant) peut désormais lire une valeur réellement produite | Projet / Sécurité | `CODE/MAIN/PRG_07_Supervision.st:28,66,183-186`, `CODE/DIAG/FB_Diag_IhmHeartbeat.st` |
| T108 | 🟡 **Interlock translation M3→Trémie (`HopperFull_OR_GateRaised_DI`)** : IO déclaré 2026-08-12 (`Device_IO_20260812.csv`, `ST_HwMachine.HopperFull_OR_GateRaised_DI`, acquis dans `HwReal`/`HwIn` sans filtrage temporel — pas encore câblé électriquement) mais **aucune logique consommatrice**. À faire dans un lot dédié M3 Translation : interdiction de l'ordre de déplacement M3 vers la cible Trémie si le signal est actif (arbitrage de mouvement, **pas** un bit `ErrorId` `FB_Safety_Translation` — décision utilisateur 2026-08-12). FB porteur (`FB_Translation` a priori), comportement exact (nouvelles demandes seulement vs mouvement en cours) à spécifier avant écriture | Projet / Translation M3 | `CODE/SUPERVISION/_TYPES/ST_HwMachine.st`, `CODE/MAIN/PRG_02_Acquisition.st` |
| T109 | 🟡 **Réflexion à trancher — polarité `Forbid*` vs `Permitted`/`Allowed`** : pour les interlocks d'**arbitrage de mouvement entre axes** (anti-télescopage, ordres gauche/droite, descente autorisée seulement en position E/maintenance — pas la chaîne safety dure `SafeStop`/`ForbidDescent`/`PowerCutOff` déjà justifiée par REX), une convention positive type `<Mouvement>Permitted`/`<Mouvement>Allowed` réduirait les doubles négations. Ne pas implémenter sans décision documentée — à trancher avant ou avec T108 | Projet / Convention | Discussion 2026-08-12, `DOC/STDS/NAMING_CONVENTION.md §Polarité des booléens I/O` |
| T110 | 🔴 **À analyser — sémantique `DriveStatusWord.0` dans Méca B (`FB_Safety_Translation`)** : REX 2026-08-14 (troubleshooting AU en full simu) — `FB_SimBench.st` (corrigé ce lot) forçait `M3_StatusWordSim` bit0 à 1 en continu à l'arrêt, alors que `PRG_05_Translation.st:421` et le commentaire `FB_SimBench.st:261` documentent ce même bit comme **"Power Ready"** (étage de puissance prêt, pas "en mouvement"). Or `FB_Safety_Translation.st:181` (Méca B, `UncommandedActiveB := DriveStatusWord.0 OR BrakeFeedback`) l'utilise comme indicateur de **mouvement non commandé** — sémantique contradictoire avec "Power Ready" qui devrait normalement rester à 1 en continu tant que le variateur est sous tension, y compris à l'arrêt. Fix appliqué ce lot = simulation seule (le bit simulé retombe à l'arrêt, corrige le symptôme). **Reste à trancher** : sur l'AC600 réel, ce bit retombe-t-il vraiment à l'arrêt stable (condition Méca B jamais fausse, code correct) ou reste-t-il à 1 (Méca B potentiellement jamais réellement testé en conditions >3s d'arrêt stable, écart réel à corriger) ? Écart complémentaire repéré dans la même zone : `AF_Partie-11/FB_Safety_Translation_v1.1.md §4` documente la condition standard comme `DriveStatusWord.0 OR NOT BrakeFeedback` (avec `NOT`), le code (`FB_Safety_Translation.st:181`) n'a pas ce `NOT` — à vérifier ensemble | Projet / Sécurité / Constructeur variateur | `CODE/TRANSLATION/FB_Safety_Translation.st:176-190`, `CODE/SIMULATION/FB_SimBench.st:261-268`, `DOC/AF/AF_Partie-11_Fonction_Translation/FB_Safety_Translation_v1.1.md §4` |
| T111 | ✅ **Résolu (2026-08-15)** — **Inversion mécanique Mou de Câble (`FB_Safety_Winch.st`)** : Remplacer l'ancienne règle erronée (`SlackCableDetected` interdisait la montée) par la règle physique réelle : en mode récupération (`SyncEnable=FALSE`), **bloquer la descente (`ForbidDescent`)** pour éviter de dérouler du câble au tambour, et **autoriser la montée (vitesse lente)** pour retendre le mou et fermer la benne | Projet / Automatisme | `CODE/TREUILS/FB_Safety_Winch.st:410-418, 595-618`, `DOC/AF/AF_Partie-10_Fonction_Winch_v2.0.md §6.5` |
| T112 | ✅ **Résolu (2026-08-15)** — **Refactor sémantique de polarité `Forbid*` ➔ `*Permit` (ou `Auth*`)** : Remplacer les sorties négatives (`ForbidAscent`, `ForbidDescent`) génératrices de doubles négations par des signaux positifs (`AscentPermit`, `DescendPermit`) pour homogénéiser la lisibilité du code et de l'IHM | Projet / Standardisation | `CODE/TREUILS/FB_Safety_Winch.st`, `CODE/MAIN/PRG_04_Treuils_Benne.st`, `DOC/AF/AF_Partie-10_Fonction_Winch_v2.0.md` |
| T113 | ✅ **Résolu (2026-08-15)** — **Synchronisation M1/M2 étagée (`FB_WinchSync.st`)** : Remplacer la coupure brutale immédiate par un asservissement en 3 zones : Zone 1 (<0.3m) = nominal ; Zone 2 (0.3m..0.8m) = **dégradation au Palier 1 (vitesse lente)** sans couper le mouvement ; Zone 3 (>1.2m) = **SafeStop** avec alarme Méca E | Projet / Automatisme | `CODE/TREUILS/FB_WinchSync.st`, `CODE/MAIN/PRG_04_Treuils_Benne.st`, `DOC/AF/AF_Partie-10_Fonction_Winch_v2.0.md §6.7` |
| T114 | ✅ **Résolu (2026-08-15)** — **Autorisation de remontée Palier 1 si Benne obstruée (`PRG_04_Treuils_Benne.st`)** : Si la benne est obstruée par un obstacle ou décalage codeur (`NOT IsClosed`), interdire la montée rapide (Paliers 2..5) mais **autoriser la remontée en Vitesse Lente (Palier 1)** pour remonter la benne en surface | Projet / Exploitation | `CODE/MAIN/PRG_04_Treuils_Benne.st`, `DOC/AF/AF_Partie-10_Fonction_Winch_v2.0.md §6.6` |
