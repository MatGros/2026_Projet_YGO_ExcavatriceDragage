...
| T96 | ✅ **Résolu (constaté lors de l'audit M3 2026-08-05)** — `M3_ThermalFeedback_DI` câblé mais jamais lu (audit nommage 2026-07-30) : disjoncteur thermique **moteur/variateur AC600**. **Preuve** : `ST_HwTranslation.M3_ThermalFeedback_DI` lu dans `PRG_02_Acquisition.st` (`HwReal`→filtrage 20 ms→`HwRealQualified`→`HwIn`), publié en diagnostic IHM par `PRG_05_Translation.st:164` (`TranslationStateHMI.DriveThermalOk`). Diagnostic seul, comme spécifié — aucune escalade `SafeStop`/`PowerCutOff`, conforme à la décision 2026-07-30 | Projet / Électricité | `CODE/SUPERVISION/_TYPES/ST_HwTranslation.st`, `CODE/MAIN/PRG_02_Acquisition.st`, `CODE/MAIN/PRG_05_Translation.st:164` |
...
| T108 | 🟡 **Interlock translation M3→Trémie (`HopperFull_OR_GateRaised_DI`)** : IO déclaré 2026-08-12 (`Device_IO_20260812.csv`, `ST_HwMachine.HopperFull_OR_GateRaised_DI`, acquis dans `HwReal`/`HwRealQualified`/`HwIn` sans filtrage temporel — pas encore câblé électriquement) mais **aucune logique consommatrice**. À faire dans un lot dédié M3 Translation : interdiction de l'ordre de déplacement M3 vers la cible Trémie si le signal est actif (arbitrage de mouvement, **pas** un bit `ErrorId` `FB_Safety_Translation` — décision utilisateur 2026-08-12). FB porteur (`FB_Translation` a priori), comportement exact (nouvelles demandes seulement vs mouvement en cours) à spécifier avant écriture | Projet / Translation M3 | `CODE/SUPERVISION/_TYPES/ST_HwMachine.st`, `CODE/MAIN/PRG_02_Acquisition.st` |
