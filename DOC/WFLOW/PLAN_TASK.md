...
| T139 | **Restructuration thématique des 77 DUT** dans `CODE/J_SUPERVISION/_TYPES/` (8 sous-dossiers thématiques) + mise à jour outillage et baseline | Projet / Convention | ⏳ | AGY-01 | **Implémenté 2026-08-19** : 77 DUT déplacés via `git mv` dans 8 dossiers (`1_TREUILS_BENNE` à `8_BANDEAU_ET_IHM`) ; G380 récursif ; `naming_baseline.json` à jour ; bundle frais ; G200 PASS ; 15/15 gates PASS (`run_all_gates.py --palier C`). Contrat : `TASK_CONTRACT_RESTRUCTURE_TYPES_DIR.yaml`. ⏳ En attente de validation orchestrateur/humain |
