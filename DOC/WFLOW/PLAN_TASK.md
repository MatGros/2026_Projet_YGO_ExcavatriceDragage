...
| T136 | **Écrire les 2 contrats d'interface FB** (`light` / `standard` + DUT `ST_FbStatus`) dans `CODE_QUALITY_STANDARDS.md` + `guard:` de vérification — **doc seul, zéro ligne `CODE/`** | Projet / Convention | ⏳ | AGY-01 | **Implémenté 2026-08-19** : (1) Contrats light et standard + struct `ST_FbStatus` (6 membres) documentés dans `CODE_QUALITY_STANDARDS.md §2quinquies` ; (2) doublon 2bis résolu ; (3) déduplication Ladder dans `AF_Partie-03` ; (4) script de garde mécanique `G315_check_fb_interface.py` créé et validé + unittests OK. Contrat validé : `TASK_CONTRACT_STANDARD_INTERFACES_FB.yaml`. **Corrigé (2026-08-20, revue T136)** : le classifieur était **type-blind** (comptait `State` au NOM, sans type ni visibilité) → 5 FB à `State` domaine (E_Diag_State, ST_Safety_Emergency_State, CycleStep…) et `FB_Cycle` (State `[LOC]`) étaient comptés « standard » à tort. Corrigé : G315 **type-aware**, DUT `ST_FbStatus` créé (`CODE/A_COMMUN/`) + existence vérifiée. **Nouveau décompte : 16 standard, 28 light, 9 exceptions documentées**. Tests mis à jour (11). ⏳ En attente de validation orchestrateur/humain |
