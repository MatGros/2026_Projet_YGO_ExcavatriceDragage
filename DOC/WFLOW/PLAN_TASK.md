...
> 🎯 **Rôle** : Document de synthèse et historique des jalons/reliquats.
> 🗂️ **Source de pilotage dynamique & opérationnel** : [`DOC/WFLOW/TASKS.yaml`](TASKS.yaml) (visualisable et filtrable dans **Data Preview** sous VS Code via la skill `.claude/skills/task-planner/`).
> 🗓️ Créé 2026-07-09 · Mis à jour 2026-08-22.
...
| T146 | **Référencement / homing des codeurs : état incohérent & suspect — mise à plat, challenge & simplification** | Projet / Sécurité / Convention | 🔒 | DSH-02 | **TÂCHE EN COURS (NON CLÔTURÉE)**. Audit 2026-08-22 : Façade `FB_Encoder` créée (Abs, Homing, Scale, Safety, Speed), mais 2 points critiques restent à traiter avant clôture : ① Interlock hauteur Translation M3 (`M3_HeightInterlockOk`) : fail-safe incomplet — un codeur non homé mais dans les bornes est cru par l'interlock ; imposer strictement la condition positive `Homed = TRUE`. ② Synchronisation de l'état `Homed` entre `FB_Encoder_Homing` et `FB_Encoder_Safety`. ③ Mise à jour de l'AF-09 §5 (acter le homing en configuration benne fermée). |
