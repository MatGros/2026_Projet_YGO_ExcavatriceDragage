...
| T118 | 🟡 IHM : quand un mouvement est bloqué par un interlock, afficher la **cause + l'action à faire** (ex. descente bloquée par Dive + benne fermée → « Ouvrir la benne ») au lieu d'un simple « mouvement interdit ». **⚠️ Différé — analyse d'abord** (session 2026-08-16) : déterminer si les **messages/textes dans les champs** existants suffisent, ou s'il faut **ajouter des voyants/infos IHM** pour que l'utilisateur comprenne. À instruire en travaillant sur MAINT_N1/N2 | Projet / IHM | `PRG_07_Supervision.st`, `FB_Hmi_BannerFormatter.st`, reliquat session 2026-08-16 — nécessite d'exposer la **cause du blocage** (pas seulement le bit permit) ; lié à T116 |
