...
| T143 | **Meca DriftGuard se déclenche au référencement sans mouvement** | Projet / Sécurité / Simulation | ⬜ | — | **Identification (2026-08-20)** : à l'appui du bouton de référencement M2 (benne ouverte/fermée) ou M1 (codeur), **sans mouvement**, MecaA se déclenche via `DriftGuardA.Violation` → PowerCutOff → contacteur coupé → mode DISABLE. Le bypass MecaA (IHM) permet le référencement. <br>**Cause probable** : `InReferencingMode := instHomingM2.Busy` (PRG_04 l.736) mal synchronisé — DriftGuardA armé (`NOT InReferencingMode`, FB_Safety_Winch l.283) au moment de l'appui → détecte une dérive transitoire. **L'ErrorId n'est pas conservé** (défaut de conception) → cause invisible au snapshot. <br>**Solution proposée** : ① vérifier/corriger la synchro `InReferencingMode` pendant le référencement ; ② conserver l'`ErrorId` (latch cause) jusqu'au Reset pour rendre la cause visible. |
