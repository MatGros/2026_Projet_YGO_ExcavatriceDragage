...
- → règle à ré-énoncer : « sens avant vitesse, fenêtre bornée `T_max` ; à expiration → chute sens + latch `ContactorStuck` + escalade définie ». **Timeout sur le filet**, pas le niveau gouvernant seul.
...
- Perte diag `ContactorStuck` Phase 0→A : `instCauses[1]` (`:123`) → `FB_Winch.Fault` retiré. **Trancher par écrit** : `FB_Safety_Winch.ContactorStuck` déclenche une **action** (`SafeStop`/`PowerCutOff`) ou juste du diag ? AC5 exige un TC mais **pas l'action protectrice**.
...
2. **Borner §3bis** — fenêtre `T_max`, timeout sur le niveau **filet**, comportement pour les 2 états de panne de `ContactorsAllOff` (DI simple voie), escalade → `ContactorStuck` + latch. **[S-M]**
...
5. **Compléter les AC** — (a) T181-01 AC3 : constantes de cadence **provisoires / essai site** ; (b) T181-01 AC4 : `DirectionDropBlocked` transitoire ≠ gouvernance ; (c) T181-05 AC10 : non-régression **par TC** ; (d) T181-02_03 AC5 : asserter l'**action protectrice** de `FB_Safety_Winch.ContactorStuck` ; (e) réconcilier `G4xx_check_direction_after_speed` avec les coupures dures légitimes (`NOT Enable`, `Fault.Error`, `PowerCutOff`). **[S]**

---

## Incertitudes à lever
- `FB_Safety_Winch.ContactorStuck` → action (`SafeStop`/`PowerCutOff`) ou diag seul ?
