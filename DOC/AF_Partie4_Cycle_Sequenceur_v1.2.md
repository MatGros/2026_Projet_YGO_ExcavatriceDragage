# 📋 Analyse Fonctionnelle — Partie 4 : Cycle & Séquenceur (v1.2)

> **Version 1.2** — 2026-07-02 : correction terminologie utilisateur — **godet → grappin**
> partout dans ce document (concept **central** ici : §6 « Cinématique grappin », interlock
> §3bis, étapes `DESCENDING_OPEN`/`DESCENDING_OPEN_DUMP`). `FB_Bucket` → `FB_Grappin`,
> `ST_BucketState` → `ST_GrappinState` (cohérence avec Partie 2 v2.6, Partie 3 v1.2). Ancienne
> version (`godet`/`FB_Bucket`) archivée dans `DOC/Archives/AF_Partie4_Cycle_Sequenceur_v1.1.md`.
> 🔧 **2026-07-02** — Terminologie : Translation → Chariot (liste I/O réelle reçue de l'utilisateur,
> terminologie officielle du matériel) — §5 renommé, `FB_Translation` → `FB_Chariot` partout — voir
> Partie 11 v1.2.
...
>
> 🔗 Dépend de : Partie 2 v2.6 (architecture), Partie 3 v1.2 (contrat FB), Partie 5 (modes).
...
## 📚 Documents liés
- **Partie 2 v2.6** — Architecture (orchestration, `FB_WinchSync`, `SafeStop`/`StartStop`, IO).
