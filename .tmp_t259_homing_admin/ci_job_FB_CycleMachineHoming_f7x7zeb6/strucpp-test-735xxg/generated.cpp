...
FB_CYCLEMACHINEHOMING::FB_CYCLEMACHINEHOMING()
    : MACHINEHOMINGSTEP(E_MACHINEHOMINGSTEP::IDLE), MACHINEHOMINGSTEPATERROR(E_MACHINEHOMINGSTEP::IDLE), MACHINEHOMINGSEQSTEP(E_MACHINEHOMINGTXSTATE::HX0_REPOS), SEQSTEP(E_MACHINEHOMINGTXSTATE::HX0_REPOS), CST_CAUSEHOMINGLOSTINMOTION(0), CST_CAUSECLIMBTIMEOUT(1), CST_CAUSEMOTIONOUTOFPHASE(2), CST_CAUSEHOMINGERRORM1(3), CST_CAUSEHOMINGERRORM2(4), CST_CAUSEHOMEAXESTIMEOUT(5), CST_VALIDATIONHOLD(300000000LL), CST_VALIDATIONPRESSHOLD(150000000LL), CST_VALIDATIONGAP(2000000000LL), CST_VALIDATIONCOUNT(3), CST_SETTLEGRACE(3000000000LL), CST_STEPSLOW(1)
{
    // Initialize variables
}

void FB_CYCLEMACHINEHOMING::operator()() {
    if (__mocked_) { __mock_state_.call_count++; return; }
    ...
    CMDWINCHM1.RUNREQUEST = false;
    CMDWINCHM1.REQASCENT = false;
    CMDWINCHM1.REQDESCEND = false;
    CMDWINCHM1.STEPTGT = 0;
    CMDWINCHM2.RUNREQUEST = false;
    CMDWINCHM2.REQASCENT = false;
    CMDWINCHM2.REQDESCEND = false;
    CMDWINCHM2.STEPTGT = 0;
    ...
    LATCHEDEDGE();
    LATCHEDEDGE.ENO = true;
    TOPLOSTEDGE.CLK = TOPPOSITIONSENSOR;
    TOPLOSTEDGE();
    TOPLOSTEDGE.ENO = true;
    ISINMOVEPHASE = ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX2_CLIMB)) | ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES));
    CYCLERUNNING = ((SEQSTEP != E_MACHINEHOMINGTXSTATE::HX0_REPOS)) & ((SEQSTEP != E_MACHINEHOMINGTXSTATE::HX1_CHOICE));
    BOTHAXESHOMED = (M1STATUS.HOMEDANDRELIABLE) & (M2STATUS.HOMEDANDRELIABLE);
    COMMITOROFFSETVALID = (COMMITPUBLISHED) | (BUCKETOFFSETVALID);
    CLIMBPERMIT = (DEADMANARMED) & (JOYSTICKPULL);
    DESCENDPERMIT = (DEADMANARMED) & (JOYSTICKPUSH);
    BUCKETPERMIT = (DEADMANARMED) & (JOYSTICKDEFLECTED);
    SETTLEGRACETIMER.IN = ((((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX2N_NEUTRAL)) | ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3N_PAUSE))) | ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX4_BUCKET_ADJUST))) | ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX5_BUCKET_COMMIT));
    SETTLEGRACETIMER.PT = CST_SETTLEGRACE;
    SETTLEGRACETIMER();
    SETTLEGRACETIMER.ENO = true;
    AXISHOMINGERROR = (M1STATUS.HOMINGERROR) | (M2STATUS.HOMINGERROR);
    MOTIONOUTOFPHASE = (((CYCLERUNNING) & (!ISINMOVEPHASE)) & (!WINCHESMECHANICALLYSTOPPED)) & (SETTLEGRACETIMER.Q);
    CLIMBTIMEDOUT = ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX2_CLIMB)) & (CLIMBTIMER.Q);
    HOMEAXESTIMEDOUT = (((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES)) & (HOMEAXESTIMER.Q)) & (!BOTHAXESHOMED);
    MODELOSTDURINGCYCLE = (CYCLERUNNING) & ((MODE != E_MODE::MAINT_N2));
    TRANSACTIONABORT = (CYCLERUNNING) & ((((((AXISHOMINGERROR) | (MOTIONOUTOFPHASE)) | (CLIMBTIMEDOUT)) | (HOMEAXESTIMEDOUT)) | (MODELOSTDURINGCYCLE)));
    AUTOARMCANDIDATE = (((((FIRSTSCANDONE) & (!MACHINEHOMED)) & (!MACHINEHOMINGFAILED)) & (!REHOMINGACKREQUIRED)) & (!HOMINGLOSSLATCHED)) & ((((MODE == E_MODE::MAINT_N1)) | ((MODE == E_MODE::MAINT_N2))));
    AUTOARMTIMER.IN = AUTOARMCANDIDATE;
    AUTOARMTIMER.PT = CST_VALIDATIONHOLD;
    AUTOARMTIMER();
    AUTOARMTIMER.ENO = true;
    VALIDATIONBTNEDGER.CLK = BTNVALIDATION;
    VALIDATIONBTNEDGER();
    VALIDATIONBTNEDGER.ENO = true;
    VALIDATIONBTNEDGEF.CLK = BTNVALIDATION;
    VALIDATIONBTNEDGEF();
    VALIDATIONBTNEDGEF.ENO = true;
    STARTEDGE.CLK = STARTREQUEST;
    STARTEDGE();
    STARTEDGE.ENO = true;
    VALIDATIONHOLDTIMER.IN = BTNVALIDATION;
    VALIDATIONHOLDTIMER.PT = CST_VALIDATIONPRESSHOLD;
    VALIDATIONHOLDTIMER();
    VALIDATIONHOLDTIMER.ENO = true;
    VALIDATIONGAPTIMER.IN = ((VALIDATIONPRESSCOUNT > 0)) & (!BTNVALIDATION);
    VALIDATIONGAPTIMER.PT = CST_VALIDATIONGAP;
    VALIDATIONGAPTIMER();
    VALIDATIONGAPTIMER.ENO = true;
    if ((BTNVALIDATION) & (VALIDATIONHOLDTIMER.Q)) {
        VALIDATIONHELDOK = true;
    }
    if ((JOYSTICKDEFLECTED) | (!WINCHESMECHANICALLYSTOPPED)) {
        VALIDATIONPRESSCOUNT = 0;
        VALIDATIONHELDOK = false;
    } else if (VALIDATIONGAPTIMER.Q) {
        VALIDATIONPRESSCOUNT = 0;
        VALIDATIONHELDOK = false;
    } else if ((VALIDATIONBTNEDGEF.Q) & (VALIDATIONHELDOK)) {
        VALIDATIONPRESSCOUNT = VALIDATIONPRESSCOUNT + 1;
        VALIDATIONHELDOK = false;
    }
    if (VALIDATIONPRESSCOUNT >= CST_VALIDATIONCOUNT) {
        EXPLICITVALIDATIONPULSE = true;
        VALIDATIONPRESSCOUNT = 0;
    }
    MACHINEHOMEDRAW = (((BOTHAXESHOMED) & (COMMITOROFFSETVALID)) & (!MACHINEHOMINGFAILED)) & (!REHOMINGACKREQUIRED);
    AXESDATUMRAW = ((BOTHAXESHOMED) & (!MACHINEHOMINGFAILED)) & (!REHOMINGACKREQUIRED);
    if (AXESDATUMRAW) {
        MACHINEWASHOMED = true;
    }
    if (((MACHINEWASHOMED) & (!AXESDATUMRAW)) & (!WINCHESMECHANICALLYSTOPPED)) {
        HOMINGLOSSLATCHED = true;
    }
    if ((!AXESDATUMRAW) & (WINCHESMECHANICALLYSTOPPED)) {
        MACHINEWASHOMED = false;
    }
    if ((HOMINGLOSSLATCHED) & (WINCHESMECHANICALLYSTOPPED)) {
        HOMINGLOSSLATCHED = false;
    }
    if ((HOMINGLOSSLATCHED) & (!REHOMINGACKREQUIRED)) {
    ...
    if (((FORCESTEPAPPLYEDGE.Q) & (CFGCOMMISSIONINGENABLE)) & ((MODE == E_MODE::MAINT_N2))) {
        FORCESTEPCLAMPED = LIMIT(static_cast<IEC_INT>(0), CFGFORCESTEPTARGET, static_cast<IEC_INT>(9));
        switch (FORCESTEPCLAMPED) {
            case 0:
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX0_REPOS;
                break;
            case 1:
    ...
    INSTCAUSES.at(CST_CAUSEHOMINGLOSTINMOTION).ACTIVE = (HOMINGLOSSLATCHED) | (REHOMINGACKREQUIRED);
    INSTCAUSES.at(CST_CAUSEHOMINGLOSTINMOTION).LATCHING = true;
    INSTCAUSES.at(CST_CAUSEHOMINGLOSTINMOTION).TEXTE = "Perte de datum machine en mouvement : re-referencer";
    INSTCAUSES.at(CST_CAUSECLIMBTIMEOUT).ACTIVE = CLIMBTIMEDOUT;
    INSTCAUSES.at(CST_CAUSECLIMBTIMEOUT).LATCHING = true;
    INSTCAUSES.at(CST_CAUSECLIMBTIMEOUT).TEXTE = "Montee au capteur haut non aboutie dans le temps imparti";
    INSTCAUSES.at(CST_CAUSEMOTIONOUTOFPHASE).ACTIVE = MOTIONOUTOFPHASE;
    INSTCAUSES.at(CST_CAUSEMOTIONOUTOFPHASE).LATCHING = true;
    INSTCAUSES.at(CST_CAUSEMOTIONOUTOFPHASE).TEXTE = "Mouvement treuil detecte hors phase autorisee du cycle";
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM1).ACTIVE = (CYCLERUNNING) & (M1STATUS.HOMINGERROR);
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM1).LATCHING = true;
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM1).TEXTE = "Erreur de homing codeur M1 pendant le referencement conjoint";
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM2).ACTIVE = (CYCLERUNNING) & (M2STATUS.HOMINGERROR);
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM2).LATCHING = true;
    INSTCAUSES.at(CST_CAUSEHOMINGERRORM2).TEXTE = "Erreur de homing codeur M2 pendant le referencement conjoint";
    INSTCAUSES.at(CST_CAUSEHOMEAXESTIMEOUT).ACTIVE = HOMEAXESTIMEDOUT;
    INSTCAUSES.at(CST_CAUSEHOMEAXESTIMEOUT).LATCHING = true;
    INSTCAUSES.at(CST_CAUSEHOMEAXESTIMEOUT).TEXTE = "Homing des axes non obtenu dans le temps imparti";
    ...
    CLIMBTIMER.IN = ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX2_CLIMB)) & ((CFG.CFGTIMEOUTCLIMB > 0LL));
    CLIMBTIMER.PT = CFG.CFGTIMEOUTCLIMB;
    CLIMBTIMER();
    CLIMBTIMER.ENO = true;
    HOMEAXESTIMER.IN = ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES)) & ((CFG.CFGTIMEOUTHOMEAXES > 0LL));
    HOMEAXESTIMER.PT = CFG.CFGTIMEOUTHOMEAXES;
    HOMEAXESTIMER();
    HOMEAXESTIMER.ENO = true;
    switch (SEQSTEP) {
        case E_MACHINEHOMINGTXSTATE::HX0_REPOS:
            if (((((STARTEDGE.Q) | (((((EXPLICITVALIDATIONPULSE) | (AUTOARMTIMER.Q))) & (!BOTHAXESHOMED)))) | ((((MACHINEWASHOMED) & (!MACHINEHOMEDRAW)) & (!BOTHAXESHOMED))))) & ((((MODE == E_MODE::MAINT_N1)) | ((MODE == E_MODE::MAINT_N2))))) {
                HOMEREQDONE = false;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX1_CHOICE;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX1_CHOICE:
            if ((MACHINEHOMED) | ((((MODE != E_MODE::MAINT_N1)) & ((MODE != E_MODE::MAINT_N2))))) {
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX0_REPOS;
            } else if ((EXPLICITVALIDATIONPULSE) & ((MODE == E_MODE::MAINT_N2))) {
                COMMITPUBLISHED = false;
                COMMITDONE = false;
                if (!TOPPOSITIONSENSOR) {
                    SEQSTEP = E_MACHINEHOMINGTXSTATE::HX2_CLIMB;
                } else {
                    SEENNEUTRAL = !JOYSTICKDEFLECTED;
                    SEQSTEP = E_MACHINEHOMINGTXSTATE::HX2N_NEUTRAL;
                }
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX2_CLIMB:
            if (CLIMBPERMIT) {
                CMDWINCHM1.RUNREQUEST = true;
                CMDWINCHM1.REQASCENT = true;
                CMDWINCHM1.STEPTGT = CST_STEPSLOW;
                CMDWINCHM2.RUNREQUEST = true;
                CMDWINCHM2.REQASCENT = true;
                CMDWINCHM2.STEPTGT = CST_STEPSLOW;
            }
            if (TRANSACTIONABORT) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if (TOPPOSITIONSENSOR) {
                SEENNEUTRAL = !JOYSTICKDEFLECTED;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX2N_NEUTRAL;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX2N_NEUTRAL:
            ...
            if (TRANSACTIONABORT) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if ((((WINCHESMECHANICALLYSTOPPED) & (SEENNEUTRAL)) & (DESCENDPERMIT)) & (TOPPOSITIONSENSOR)) {
                HOMEREQDONE = false;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES:
            if (DESCENDPERMIT) {
                CMDWINCHM1.RUNREQUEST = true;
                CMDWINCHM1.REQDESCEND = true;
                CMDWINCHM1.STEPTGT = CST_STEPSLOW;
                CMDWINCHM2.RUNREQUEST = true;
                CMDWINCHM2.REQDESCEND = true;
                CMDWINCHM2.STEPTGT = CST_STEPSLOW;
            }
            if ((TOPLOSTEDGE.Q) & (!HOMEREQDONE)) {
            ...
            if (TRANSACTIONABORT) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if (((!TOPPOSITIONSENSOR) & (M1STATUS.HOMEDANDRELIABLE)) & (M2STATUS.HOMEDANDRELIABLE)) {
                SEENNEUTRAL = !JOYSTICKDEFLECTED;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX3N_PAUSE;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX3N_PAUSE:
            if (!JOYSTICKDEFLECTED) {
                SEENNEUTRAL = true;
            }
            if (TRANSACTIONABORT) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if ((WINCHESMECHANICALLYSTOPPED) & (SEENNEUTRAL)) {
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX4_BUCKET_ADJUST;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX4_BUCKET_ADJUST:
            if ((BUCKETPERMIT) & (JOYSTICKPULL)) {
                CMDBUCKETCLOSE = true;
            } else if ((BUCKETPERMIT) & (JOYSTICKPUSH)) {
                CMDBUCKETOPEN = true;
            }
            if (((AXISHOMINGERROR) | (MOTIONOUTOFPHASE)) | (MODELOSTDURINGCYCLE)) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if (((EXPLICITVALIDATIONPULSE) & (!JOYSTICKDEFLECTED)) & (WINCHESMECHANICALLYSTOPPED)) {
                PENDINGCLOSE = true;
                COMMITDONE = false;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HX5_BUCKET_COMMIT;
            }
            break;
        case E_MACHINEHOMINGTXSTATE::HX5_BUCKET_COMMIT:
            if ((((AXISHOMINGERROR) | (!BOTHAXESHOMED)) | (MOTIONOUTOFPHASE)) | (MODELOSTDURINGCYCLE)) {
                MACHINEHOMINGFAILED = true;
                SEQSTEP = E_MACHINEHOMINGTXSTATE::HXF_FAILED;
            } else if (!COMMITDONE) {
            ...
            break;
        case E_MACHINEHOMINGTXSTATE::HX6_HOMED:
            SEQSTEP = E_MACHINEHOMINGTXSTATE::HX0_REPOS;
            break;
        case E_MACHINEHOMINGTXSTATE::HXF_FAILED:
            break;
    }
    CYCLERUNNING = ((SEQSTEP != E_MACHINEHOMINGTXSTATE::HX0_REPOS)) & ((SEQSTEP != E_MACHINEHOMINGTXSTATE::HX1_CHOICE));
    ...
    } else if ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX2N_NEUTRAL)) {
    ...
    } else if ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3_HOME_AXES)) {
    ...
    } else if ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX3N_PAUSE)) {
        MACHINEHOMINGSTEP = E_MACHINEHOMINGSTEP::NEED_MECHANICAL_STOP;
        MACHINEHOMINGINSTRUCTION = "HX3N - RefHoming JOY au neutre (arret avant benne)";
    } else if ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX4_BUCKET_ADJUST)) {
    ...
    } else if (((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX5_BUCKET_COMMIT)) | ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX6_HOMED))) {
        MACHINEHOMINGSTEP = E_MACHINEHOMINGSTEP::VALID;
        MACHINEHOMINGINSTRUCTION = "HX5 - RefHoming Etat benne enregistre - fin homing";
    } else if ((SEQSTEP == E_MACHINEHOMINGTXSTATE::HX1_CHOICE)) {
        if (MODE != E_MODE::MAINT_N2) {
    ...
    } else if ((!M1STATUS.HOMEDANDRELIABLE) & (!M2STATUS.HOMEDANDRELIABLE)) {
