...
"""
clean_and_apply.py
Restaure une base propre et injecte de façon idempotente et sans duplication :
- Le CSS du cockpit ST multi-onglets
- Le bouton Header vers la modale ST
- La barre de forçage de mise en service (§4bis)
- Le cockpit enrichi à 4 onglets
- La modale complète du code ST (FB_CycleMachineHoming.st, 652 lignes)
- Les données ST exactes dans HOMING_STEPS
- Les fonctions JavaScript nécessaires
- Vérification syntaxique avec Node.js
"""
...
def generate_clean_js(steps_data, regions, st_full):
    regions_json = []
    ...
    # JSON encode cleanly
    st_full_js = json.dumps(st_full)
    st_reg_js = json.dumps(regions_json)

    js = f"""
    // --- NOUVELLES FONCTIONS COCKPIT MULTI-ONGLETS & CODE ST (T364) ---
    let currentCockpitTab = 'tab-role';
    let retryCountSimulation = 0;
    const ST_FULL_SOURCE = {st_full_js};
    const ST_REGIONS = {st_reg_js};
...
def clean_base_html(src):
    # Strip any previously injected blocks
    src = re.sub(r'/\* --- ENRICHISSEMENT COCKPIT ST & MULTI-ONGLETS.*?(?=\s*</style>)', '', src, flags=re.DOTALL)
    src = re.sub(r'<!-- Commissioning Step Forcing Bar.*?</div>\s*</div>', '', src, flags=re.DOTALL)
    src = re.sub(r'<button class="nav-tab" onclick="openStModal\(\)".*?</button>\s*', '', src, flags=re.DOTALL)
    src = re.sub(r'<!-- Full ST Automate Code Modal -->\s*<div id="st-modal".*?</div>\s*</div>\s*', '', src, flags=re.DOTALL)
    src = re.sub(r'// --- NOUVELLES FONCTIONS COCKPIT MULTI-ONGLETS.*?window\.addEventListener\("DOMContentLoaded", init\);', 'window.addEventListener("DOMContentLoaded", init);', src, flags=re.DOTALL)
    src = src.replace("updateGranularTelemetry(step);\n      updateStepCockpitTabs(step);", "updateGranularTelemetry(step);")
    return src

def build_enriched_html():
    steps_data, regions, st_full = enrich_homing_st.main()

    with open(HTML_LUMINEUX, "r", encoding="utf-8") as f:
        src = f.read()

    # 0. Clean any previous injection
    src = clean_base_html(src)
    ...
    if pattern_startup.search(src):
        ...
        print("Avertissement : startup-state-bar non trouvé")
    ...
    if pattern_cockpit.search(src):
        ...
        print("Avertissement : cockpit card non trouvé")

    # 5. Inject Full ST Modal right before </body>
    src = src.replace("</body>", FULL_ST_MODAL_HTML + "\n</body>")

    # 6. Replace HOMING_STEPS in <script> with lambda to protect backslashes
    ...
    if pattern_steps.search(src):
        ...
        print("Avertissement : HOMING_STEPS non trouvé")

    # 7. Add JS Additions into <script>
    js_additions = generate_clean_js(steps_data, regions, st_full)
    
    # Hook updateStepCockpitTabs into updateView
    src = src.replace("updateGranularTelemetry(step);", "updateGranularTelemetry(step);\n      updateStepCockpitTabs(step);")

    # Append js_additions right before window.addEventListener("DOMContentLoaded", init);
    ...
    parser = MyParser()
    parser.feed(src)
    print("-> Validation syntaxique HTMLParser : PASS (0 erreur)")

    # JS Syntax Validation using Node
    scripts = re.findall(r'<script>(.*?)</script>', src, re.DOTALL)
    if scripts:
        tmp_js = os.path.join(ROOT_DIR, "TOOLS", "GRAFCET_GENERATOR", "__temp_check.js")
        ...
        if os.path.exists(tmp_js):
            os.remove(tmp_js)
        if res.returncode != 0:
            print("ERREUR NODE SYNTAX:", res.stderr)
            raise SyntaxError(res.stderr)
        else:
            print("-> Validation syntaxique Node.js : PASS (JavaScript 100% valide)")

    return src

def main():
    print("Génération et nettoyage idempotents...")
    full_html = build_enriched_html()
    ...
    print("=== TERMINE AVEC SUCCES POUR LES DEUX FICHIERS ===")
