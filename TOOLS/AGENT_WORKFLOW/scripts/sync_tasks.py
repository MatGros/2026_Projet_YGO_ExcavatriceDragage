...
<body>
    ...
    <script>
        ...
        // 🔄 Chargement automatique en direct depuis TASKS.yaml (rafraîchissement automatique au F5)
        (async function autoLoadFreshYaml() {{
            try {{
                const res = await fetch('TASKS.yaml?' + Date.now());
                if (res.ok) {{
                    const text = await res.text();
                ...
                // Fallback silencieux sur defaultTasks si protocole file:// strict
            }}
        }})();
    </script>
</body>
