...
@pytest.mark.ci_fb
def test_F_MODES():
    """Test global du domaine F_MODES (compilation parallèle multi-cœurs)."""
    cmd = [sys.executable, str(RUN_TESTS_PY), "--domain", "F_MODES", "--fast", "-j", "1"]
    proc = subprocess.run(
        cmd,
        capture_output=True,
        text=True,
    ...
    if proc.returncode != 0:
        error_msg = f"Échec des tests pour le domaine F_MODES (code {proc.returncode}) :\n"
