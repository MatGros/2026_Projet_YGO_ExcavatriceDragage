...
FB_WINCHSPEEDLEARNING::FB_WINCHSPEEDLEARNING()
    : CST_AXECOUNT(2), CST_SENSCOUNT(2), CST_CHARGECOUNT(2), CST_PALIERCOUNT(5), CST_IDXMONTEE(1), CST_IDXDESCENTE(2), CST_IDXVIDE(1), CST_IDXCHARGE(2)
{
    // Initialize variables
}

void FB_WINCHSPEEDLEARNING::operator()() {
    if (__mocked_) { __mock_state_.call_count++; return; }
    READY = ENABLE;
    LEARNING = false;
    LAMPLEARN = false;
    LEARNREQUESTED = (ENABLE) & (LEARNSTART);
    MEASUREUSABLE = (MEASUREDSPEEDVALID) & (STABLEFORLEARN);
    SENSACTIVE = (DIRECTION != 0);
    STEPINRANGE = ((STEPNUMBER >= 1)) & ((STEPNUMBER <= CST_PALIERCOUNT));
    AXEINRANGE = ((WINCHID >= 1)) & ((WINCHID <= CST_AXECOUNT));
    CONTEXTVALID = ((SENSACTIVE) & (STEPINRANGE)) & (AXEINRANGE);
    COLLECTALLOWED = ((LEARNREQUESTED) & (MEASUREUSABLE)) & (CONTEXTVALID);
    if (COLLECTALLOWED) {
        if (DIRECTION > 0) {
            SENSIDX = CST_IDXMONTEE;
        } else {
            SENSIDX = CST_IDXDESCENTE;
        }
        if (LOADPRESENT) {
            CHARGEIDX = CST_IDXCHARGE;
        } else {
            CHARGEIDX = CST_IDXVIDE;
        }
        SPEEDPLAUSIBLE = ((MEASUREDSPEEDMPS >= CONFIG.MINSPEEDMPS.at(STEPNUMBER))) & ((MEASUREDSPEEDMPS <= CONFIG.MAXSPEEDMPS.at(STEPNUMBER)));
        CURRENTCELL = TABLE.CELL.at(WINCHID).at(SENSIDX).at(CHARGEIDX).at(STEPNUMBER);
        if (SPEEDPLAUSIBLE) {
            if (CURRENTCELL.SAMPLECOUNT == 0) {
                CURRENTCELL.SPEEDMPS = MEASUREDSPEEDMPS;
                CURRENTCELL.SAMPLECOUNT = 1;
            } else {
                CURRENTCELL.SPEEDMPS = (CURRENTCELL.SPEEDMPS * TO_REAL(CURRENTCELL.SAMPLECOUNT) + MEASUREDSPEEDMPS) / TO_REAL(CURRENTCELL.SAMPLECOUNT + 1);
        ...
        TABLE.CELL.at(WINCHID).at(SENSIDX).at(CHARGEIDX).at(STEPNUMBER) = CURRENTCELL;
        LEARNING = true;
        LAMPLEARN = true;
    }
    CELLSTOTAL = CST_AXECOUNT * CST_SENSCOUNT * CST_CHARGECOUNT * CST_PALIERCOUNT;
    CELLSFILLED = 0;
    for (AXEIDX = 1; AXEIDX <= CST_AXECOUNT; AXEIDX++) {
        for (SENSLOOPIDX = 1; SENSLOOPIDX <= CST_SENSCOUNT; SENSLOOPIDX++) {
            for (CHARGELOOPIDX = 1; CHARGELOOPIDX <= CST_CHARGECOUNT; CHARGELOOPIDX++) {
                for (PALIERIDX = 1; PALIERIDX <= CST_PALIERCOUNT; PALIERIDX++) {
                    if (TABLE.CELL.at(AXEIDX).at(SENSLOOPIDX).at(CHARGELOOPIDX).at(PALIERIDX).VALID) {
